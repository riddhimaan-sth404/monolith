...
impl EtwManager {
    ...
    unsafe fn run_session(
        ...
        let session_name: Vec<u16> = OsStrExt::encode_wide(
                OsString::from(SESSION_NAME).as_os_str(),
            )
        ...
        let source_id = GUID {
            data1: u32::from_ne_bytes([uuid_bytes[0], uuid_bytes[1], uuid_bytes[2], uuid_bytes[3]]),
            data2: u16::from_ne_bytes([uuid_bytes[4], uuid_bytes[5]]),
            data3: u16::from_ne_bytes([uuid_bytes[6], uuid_bytes[7]]),
            data4: [uuid_bytes[8], uuid_bytes[9], uuid_bytes[10], uuid_bytes[11], uuid_bytes[12], uuid_bytes[13], uuid_bytes[14], uuid_bytes[15]],
        };
        ...
        ProcessTrace(std::ptr::addr_of!(trace_handle), 1, std::ptr::null(), std::ptr::null_mut());
    ...
    unsafe extern "system" fn dispatch_callback(event_record: *mut EVENT_RECORD) { unsafe {
...
unsafe fn stop_session(handle: &CONTROLTRACE_HANDLE, session_name: &[u16]) { unsafe {
    ...
    let _ = ControlTraceW(*handle, session_name.as_ptr(), properties, EVENT_TRACE_CONTROL_STOP);
}}
