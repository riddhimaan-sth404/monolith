...
rule APT_Lazarus_Aug18_1 {
   ...
   strings:
      $s1 = "mws2_32.dll" fullword ascii 
      $s2 = "%s.bat" fullword ascii 
      $s3 = "%s%s%s \"%s > %s 2>&1\"" fullword ascii
      $s4 = "Microsoft Corporation. All rights reserved." fullword ascii 
      $s5 = "ping 127.0.0.1 -n 3" fullword ascii 
condition:
    any of them
}
...
rule APT_Lazarus_Aug18_2 {
   ...
   strings:
      $s1 = "vAdvapi32.dll" fullword ascii 
      $s2 = "lws2_32.dll" fullword ascii 
      $s3 = "%s %s > \"%s\" 2>&1" fullword ascii
      $s4 = "Not Service" fullword ascii 
      $s5 = "ping 127.0.0.1 -n 3" fullword ascii 
condition:
    any of them
}
