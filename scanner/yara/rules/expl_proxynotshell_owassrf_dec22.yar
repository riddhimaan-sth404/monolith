...
rule EXPL_LOG_ProxyNotShell_OWASSRF_PowerShell_Proxy_Log_Dec22_1 {
   ...
   strings:
      $s1 = "/owa/mastermailbox%40outlook.com/powershell" ascii 

      $sa1 = " 200 " ascii 
      $sa2 = " POST " ascii 

      // based on filters found in CrowdStrikes script https://github.com/CrowdStrike/OWASSRF/blob/main/Rps_Http-IOC.ps1
      $fp1 = "ClientInfo" ascii fullword
      $fp2 = "Microsoft WinRM Client" ascii fullword
      $fp3 = "Exchange BackEnd Probes" ascii fullword
condition:
...
rule EXPL_LOG_ProxyNotShell_OWASSRF_PowerShell_Proxy_Log_Dec22_2 {
   ...
   strings:
      $sr1 = / \/owa\/[^\/\s]{1,30}(%40|@)[^\/\s\.]{1,30}\.[^\/\s]{2,3}\/powershell / ascii
...
rule EXPL_LOG_ProxyNotShell_OWASSRF_PowerShell_Proxy_Log_Dec22_3 {
   ...
   strings:
      $sa1 = " POST /powershell - 444 " ascii 
      $sa2 = " POST /Powershell - 444 " ascii 
      $sb1 = " - 200 0 0 2" ascii 
...
rule EXPL_LOG_ProxyNotShell_PowerShell_Proxy_Log_Dec22_1 {
   ...
   strings:
      $re1 = /,\/[Pp][Oo][Ww][Ee][Rr][Ss][Hh][Ee][Ll][Ll][^\n]{0,50},Kerberos,true,[^\n]{0,50},200,0,,,,[^\n]{0,2000};OnEndRequest\.End\.ContentType=application\/soap\+xml charset UTF-8;S:ServiceCommonMetadata\.HttpMethod=POST;/ ascii
