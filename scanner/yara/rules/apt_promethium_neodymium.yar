...
rule PROMETHIUM_NEODYMIUM_Malware_1 {
   ...
   strings:
      $s1 = "c:\\Windows\\system32\\syswindxr32.dll" fullword ascii 
      $s2 = "c:\\windows\\temp\\TrueCrypt-Setup-7.1a-tamindir.exe" fullword ascii 
      $s3 = "%s\\ssleay32.dll" fullword ascii 
      $s4 = "%s\\libeay32.dll" fullword ascii 
      $s5 = "%s\\fprot32.exe" fullword ascii 
condition:
    any of them
}
...
rule PROMETHIUM_NEODYMIUM_Malware_4 {
   ...
   strings:
      $s1 = "c:\\windows\\temp\\winrar.exe" fullword ascii 
      ...
      $s8 = "Windows Index Services" fullword ascii 
condition:
    any of them
}
...
rule PROMETHIUM_NEODYMIUM_Malware_5 {
   ...
   strings:
      $s1 = "Winxsys.exe" fullword ascii 
      $s2 = "%s\\ssleay32.dll" fullword ascii 
      $s3 = "%s\\libeay32.dll" fullword ascii 
      $s4 = "Windows Index Services" fullword ascii 
      $s5 = "<F RAT" fullword ascii
      $s6 = "WININDX-088FA840-B10D-11D3-BC36-006067709674" fullword ascii 
condition:
    any of them
}
...
rule PROMETHIUM_NEODYMIUM_Malware_6 {
   ...
   strings:
      $s1 = "c:\\Windows\\system32\\syswindxr32.dll" fullword ascii 
      $s2 = "c:\\windows\\temp\\TrueCrypt-7.2.exe" fullword ascii 
      $s3 = "%s\\ssleay32.dll" fullword ascii 
      $s4 = "%s\\libeay32.dll" fullword ascii 
      $s5 = "%s\\fprot32.exe" fullword ascii 
      $s6 = "Windows Index Services" fullword ascii 
condition:
    any of them
}
