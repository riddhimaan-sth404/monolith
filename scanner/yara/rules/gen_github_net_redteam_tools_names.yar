...
rule HKTL_NET_NAME_FakeFileMaker {
    ...
    strings:
        $name = "FakeFileMaker" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
...
rule HKTL_NET_NAME_ADCollector {
    meta:
        ...
        modified = "2022-09-15"
    strings:
        $s_name = "ADCollector" ascii 
        $s_compile = "AssemblyTitle" ascii 

        $fp1 = "Symantec Threat Defense" ascii 
condition:
...
rule HKTL_NET_AdCollector_Sep22_1 {
   ...
   strings:
      $x1 = "ADCollector.exe --SPNs --Term key --Acls 'CN=Domain Admins,CN=Users,DC=lab,DC=local'" ascii fullword
      $s1 = "ADCollector.exe" ascii fullword
      $s2 = "ENCRYPTED_TEXT_PASSWORD_ALLOWED" ascii fullword
      $s3 = "\\Policies\\{31B2F340-016D-11D2-945F-00C04FB984F9}\\MACHINE\\Microsoft\\Windows NT\\SecEdit\\GptTmpl.inf" ascii 
      $s4 = "[-] Password Does Not Expire Accounts:" ascii 
      $s5 = "  * runAs:       {0}" ascii fullword
condition:
    any of them
}
...
rule HKTL_NET_NAME_MaliciousClickOnceGenerator {
    ...
    strings:
        $name = "MaliciousClickOnceGenerator" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_RevengeRAT_Stub_CSsharp {
    ...
    strings:
        $name = "RevengeRAT-Stub-CSsharp" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_AmsiBypass {
    ...
    strings:
        $s_name = "AmsiBypass" ascii 
        $s_compile = "AssemblyTitle" ascii 

        $fp1 = "Adaptive Threat Protection" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of ($s*)
        and not 1 of ($fp*)
}
...
rule HKTL_NET_NAME_SharpCat {
    ...
    strings:
        $name = "SharpCat" ascii fullword
        $compile = "AssemblyTitle" ascii fullword
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_Ghostpack_CompiledBinaries {
    ...
    strings:
        $name = "Ghostpack-CompiledBinaries" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_NativePayload_Reverse_tcp {
    ...
    strings:
        $name = "NativePayload_Reverse_tcp" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_RAT_NjRat_0_7d_modded_source_code {
    ...
    strings:
        $name = "RAT-NjRat-0.7d-modded-source-code" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_NativePayload_IP6DNS {
    ...
    strings:
        $name = "NativePayload_IP6DNS" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_Infrastructure_Assessment {
    ...
    strings:
        $name = "Infrastructure-Assessment" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_gray_hat_csharp_code {
    ...
    strings:
        $name = "gray_hat_csharp_code" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_NativePayload_ReverseShell {
    ...
    strings:
        $name = "NativePayload_ReverseShell" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_SharpOffensiveShell {
    ...
    strings:
        $name = "SharpOffensiveShell" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_RemoteProcessInjection {
    ...
    strings:
        $name = "RemoteProcessInjection" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
...
rule HKTL_NET_NAME_xbapAppWhitelistBypassPOC {
    ...
    strings:
        $name = "xbapAppWhitelistBypassPOC" ascii 
        $compile = "AssemblyTitle" ascii 
condition:
        (uint16(0) == 0x5A4D and uint32(uint32(0x3C)) == 0x00004550) and all of them
}
