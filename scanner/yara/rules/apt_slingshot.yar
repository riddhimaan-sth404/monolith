...
rule Slingshot_APT_Spork_Downloader {
   ...
   strings:
      $s1 = "Usage: spork -c IP:PORT" fullword ascii 
...
rule Slingshot_APT_Minisling {
   ...
   strings:
      $s1 = "{6D29520B-F138-442e-B29F-A4E7140F33DE}" fullword ascii 
condition:
    any of them
}
...
rule Slingshot_APT_Malware_1 {
   ...
   strings:
      ...
      $s2 = "BogusDll." ascii
      $s3 = "smsvcrt -h 0x%p" fullword ascii 
condition:
    any of them
}
...
rule Slingshot_APT_Malware_2 {
   ...
   strings:
      $x1 = "\\\\?\\c:\\RECYCLER\\S-1-5-21-2225084468-623340172-1005306204-500\\INFO5" fullword ascii 
      $x_slingshot = {09 46 BE 57 42 DD 70 35 5E }

      $s1 = "Opening service %s for stop access failed.#" fullword ascii 
      $s2 = "LanMan setting <%s> is ignored because system has a higher value already." fullword ascii 
      $s3 = "\\DosDevices\\amxpci" ascii 
...
rule Slingshot_APT_Malware_4 {
   ...
   strings:
      $x1 = "Ss -a 4104 -s 257092 -o 8 -l 406016 -r 4096 -z 315440" fullword ascii 

      $s1 = "Slingshot" fullword ascii
      $s2 = "\\\\?\\e:\\$Recycle.Bin\\" ascii 
