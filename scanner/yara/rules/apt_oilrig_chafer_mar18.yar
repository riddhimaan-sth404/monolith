...
rule Chafer_Exploit_Copyright_2017 {
   ...
   strings:
      $x1 = "test3 Internet Server Extension" fullword ascii 
      $x2 = "Copyright (C) 2017 Exploit" fullword ascii 
...
rule Oilrig_Myrtille {
   ...
   strings:
      ...
      $x2 = "Failed to notify rdp client process exit (MyrtilleAppPool down?), remote session {0} ({1})" fullword ascii 
      $x3 = "Started rdp client process, remote session {0}" fullword ascii 
condition:
    any of them
}
...
rule Chafer_Packed_Mimikatz {
   ...
   strings:
      $s1 = "Windows Security Credentials" fullword ascii 
      $s2 = "Minisoft" fullword ascii 
      $x1 = "Copyright (c) 2014 - 2015 Minisoft" fullword ascii 
condition:
    any of them
}
