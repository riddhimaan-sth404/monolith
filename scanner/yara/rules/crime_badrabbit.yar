...
rule BadRabbit_Gen {
   ...
   strings:
      $x1 = "schtasks /Create /SC ONCE /TN viserion_%u /RU SYSTEM /TR \"%ws\" /ST" fullword ascii
      $x2 = "schtasks /Create /RU SYSTEM /SC ONSTART /TN rhaegal /TR \"%ws /C Start \\\"\\\" \\\"%wsdispci.exe\\\"" fullword ascii
      $x3 = "C:\\Windows\\infpub.dat" fullword ascii 
      $x4 = "C:\\Windows\\cscc.dat" fullword ascii 

      $s1 = "need to do is submit the payment and get the decryption password." fullword ascii
      $s2 = "\\\\.\\GLOBALROOT\\ArcName\\multi(0)disk(0)rdisk(0)partition(1)" fullword ascii 
      $s3 = "\\\\.\\pipe\\%ws" fullword ascii 
      $s4 = "fsutil usn deletejournal /D %c:" fullword ascii 
      $s5 = "Run DECRYPT app at your desktop after system boot" fullword ascii
      $s6 = "Files decryption completed" fullword ascii 
      $s7 = "Disable your anti-virus and anti-malware programs" fullword ascii 
      $s8 = "SYSTEM\\CurrentControlSet\\services\\%ws" fullword ascii 
      $s9 = "process call create \"C:\\Windows\\System32\\rundll32.exe" fullword ascii
      $s10 = "%ws C:\\Windows\\%ws,#1 %ws" fullword ascii 
condition:
    any of them
}
...
rule BadRabbit_Mimikatz_Comp {
   ...
   strings:
      $s1 = "%lS%lS%lS:%lS" fullword ascii 
      $s2 = "lsasrv" fullword ascii 
