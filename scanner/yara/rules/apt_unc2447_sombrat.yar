...
rule APT_UNC2447_MAL_RANSOM_HelloKitty_May21_1 {
   ...
   strings:
      ...
      $s1 = "select * from Win32_ShadowCopy" ascii fullword
      $s2 = "bootfont.bin" ascii fullword
      $s3 = "DECRYPT_NOTE.txt" ascii fullword
      $s4 = ".onion" ascii 
...
rule APT_UNC2447_MAL_RANSOM_HelloKitty_May21_2 {
   ...
   strings:
      ...
      $s1 = "HelloKittyMutex" ascii 
      $s2 = "%s\\read_me_lkd.txt" ascii fullword
      $s3 = "/C ping 127.0.0.1 & del %s" ascii fullword
...
rule APT_UNC2447_PS1_WARPRISM_May21_1 {
   ...
   strings:
      ...
      $s1 = "[DllImport(\"kernel32.dll\")]public static extern IntPtr VirtualAlloc(IntPtr " ascii
      $s2 = "[Runtime.InteropServices.Marshal]::Copy($" ascii 
      $s3 = "[System.Diagnostics.Process]::Start((-join(" ascii 
condition:
    any of them
}
