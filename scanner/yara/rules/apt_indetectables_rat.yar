...
rule Indetectables_RAT {
	...
	strings:
		$s1 = "Coded By M3" fullword ascii 
		$s2 = "Stub Undetector M3" fullword ascii 
		$s3 = "www.webmenegatti.com.br" ascii 
		$s4 = "M3n3gatt1" fullword ascii 
		$s5 = "TheMisterFUD" fullword ascii 
...
rule BergSilva_Malware {
	...
	strings:
		$x1 = "C:\\Users\\Berg Silva\\Desktop\\" ascii 
		$x2 = "URLDownloadToFileA 0, \"https://dl.dropbox.com/u/105015858/nome.exe\", \"c:\\nome.exe\", 0, 0" fullword ascii

		$s1 = " Process.Start (Path.GetTempPath() & \"name\" & \".exe\") 'start server baixado" fullword ascii
		$s2 = "FileDelete(@TempDir & \"\\nome.exe\") ;Deleta o Arquivo para que possa ser executado normalmente" fullword ascii
		$s3 = " Lib \"\\WINDOWS\\system32\\UsEr32.dLl\"" fullword ascii
		$s4 = "$Directory = @TempDir & \"\\nome.exe\" ;Define a variavel" fullword ascii
		$s5 = "https://dl.dropbox.com/u/105015858" ascii 
condition:
