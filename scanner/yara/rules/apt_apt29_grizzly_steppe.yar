...
rule GRIZZLY_STEPPE_Malware_1 {
   ...
   strings:
      ...
      $s2 = "Repeat last find command)Replace specific text with different text" fullword ascii 
      $s3 = "l\\Processor(0)\\% Processor Time" fullword ascii 
      $s6 = "Self Process" fullword ascii 
      $s7 = "Default Process" fullword ascii 
      $s8 = "Star Polk.exe" fullword ascii 
condition:
    any of them
}
...
rule APT_APT29_wellmess_dotnet_unique_strings {
   ...
   strings:
      $s1 = "HealthInterval" ascii 
      $s2 = "Hello from Proxy" ascii 
      $s3 = "Start bot:" ascii 
