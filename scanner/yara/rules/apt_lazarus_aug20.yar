...
rule APT_NK_Lazarus_Network_Backdoor_Unpacked {
   ...
   strings:
      $str_netsh_1 = "netsh firewall add portopening TCP %d" ascii 
      $str_netsh_2 = "netsh firewall delete portopening TCP %d" ascii 
      $str_mask_1 = "cmd.exe /c \"%s >> %s 2>&1\"" ascii
      $str_mask_2 = "cmd.exe /c \"%s 2>> %s\"" ascii
      $str_mask_3 = "%s\\%s\\%s" ascii 
      $str_other_1 = "perflog.dat" ascii 
      $str_other_2 = "perflog.evt" ascii 
      $str_other_3 = "cbstc.log" ascii 
