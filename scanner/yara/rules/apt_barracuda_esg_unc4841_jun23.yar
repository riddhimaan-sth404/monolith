...
rule APT_UNC4841_ESG_Barracuda_CVE_2023_2868_Forensic_Artifacts_Jun23_1 {
   meta:
...
rule APT_MAL_UNC4841_SEASPY_LUA_Jun23_1 {
   ...
   strings:
      ...
      $xe1 = "os.execute('rverify'..' /tmp/'..attachment:filename())" ascii 
      $xe2 = "log.debug(\"--- opening archive [%s], mimetype [%s]\", tmpfile" ascii 
condition:
    any of them
}
...
rule SUSP_FScan_Port_Scanner_Output_Jun23 {
   meta:
...
rule SUSP_PY_Shell_Spawn_Jun23_1 {
   meta:
...
rule APT_MAL_LNX_Hunting_Linux_WHIRLPOOL_1 {
    ...
    strings:
        $s1 = "error -1 exit" fullword ascii
        $s2 = "create socket error: %s(error: %d)\n" fullword ascii
        $s3 = "connect error: %s(error: %d)\n" fullword ascii
        $s4 = {C7 00 20 32 3E 26 66 C7 40 04 31 00}
        $c1 = "plain_connect" fullword ascii
        $c2 = "ssl_connect" fullword ascii
        $c3 = "SSLShell.c" fullword ascii
condition:
    any of them
}
