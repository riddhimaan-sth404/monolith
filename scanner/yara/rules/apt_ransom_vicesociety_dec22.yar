...
rule APT_MAL_RANSOM_ViceSociety_Chily_Jan23_1 {
   ...
   strings:
      ...
      $s3 = "for /F \"tokens=*\" %1 in ('wevtutil.exe el') DO wevtutil.exe cl \"%1\"" ascii fullword
      $s4 = "cd %userprofile%\\documents\\" ascii 
      $s5 = "noise.bmp" ascii fullword
      $s6 = " Execution time: %fms (1sec=1000ms)" ascii fullword
      $s7 = "/c vssadmin.exe Delete Shadows /All /Quiet" ascii fullword
