...
rule BronzeButler_Daserf_Delphi_1 {
   ...
   strings:
      ...
      $s5 = " InjectPr" ascii
      $s6 = "Write$Error creating variant or safe array\x1fInvalid argument to time encode" fullword ascii 
...
rule BronzeButler_Daserf_C_1 {
   ...
   strings:
      ...
      $b3 = "DRIVE_REMOTE" fullword ascii 
...
rule BronzeButler_UACBypass_1 {
   ...
   strings:
      ...
      $x2 = "%programfiles%internet exploreriexplore.exe" fullword ascii 
      $x3 = "Elevation:Administrator!new:{3ad055" fullword ascii 
      $x4 = "BypassUac.pdb" fullword ascii
      $x5 = "[bypassUAC] started X64" fullword ascii 
      $x6 = "[bypassUAC] started X86" fullword ascii 
condition:
    any of them
}
...
rule BronzeButler_RarStar_1 {
   ...
   strings:
      $s1 = "Mozilla/4.0+(compatible;+MSIE+8.0;+Windows+NT+6.0;+SV1)" fullword ascii 
      $s2 = "http://www.google.co.jp" fullword ascii 
