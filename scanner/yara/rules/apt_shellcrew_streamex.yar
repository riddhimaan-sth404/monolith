...
rule StreamEx_ShellCrew {
   ...
   strings:
      ...
      $c = "greendll"
      $d = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36" ascii 
...
rule ShellCrew_StreamEx_1 {
   ...
   strings:
      ...
      $s7 = "%s\\AdobeBak\\Proc.dat" fullword ascii 
      $s8 = "e:\\workspace\\boar" fullword ascii
      $s12 = "$\\data.ini" fullword ascii 
condition:
    any of them
}
