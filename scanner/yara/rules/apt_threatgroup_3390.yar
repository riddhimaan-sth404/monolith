...
rule HttpBrowser_RAT_Sample1 {
	...
	strings:
		$s0 = "update.hancominc.com" fullword ascii 
condition:
    any of them
}
...
rule HttpBrowser_RAT_Sample2 {
	...
	strings:
		$s0 = "nKERNEL32.DLL" fullword ascii 
		$s1 = "WUSER32.DLL" fullword ascii 
		$s2 = "mscoree.dll" fullword ascii 
...
rule HttpBrowser_RAT_Gen {
	...
	strings:
		$s0 = "%d|%s|%04d/%02d/%02d %02d:%02d:%02d|%ld|%d" fullword ascii 
		$s1 = "HttpBrowser/1.0" fullword ascii 
		$s2 = "set cmd : %s" ascii fullword
		$s3 = "\\config.ini" ascii fullword
condition:
    any of them
}
...
rule PlugX_NvSmartMax_Gen {
	...
	strings:
		...
		$s7 = "r%.*s(%d)%s" fullword ascii 
		$s8 = " %s CRC " fullword ascii 
