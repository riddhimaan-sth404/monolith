...
rule FreeMilk_APT_Mal_1 {
   ...
   strings:
      ...
      $x3 = "Windows-KB271854-x86.exe" fullword ascii 

      $s1 = "Windows-KB275122-x86.exe" fullword ascii 
      $s2 = "\\wsatra.tmp" ascii 
      $s3 = "%s\\Rar0tmpExtra%d.rtf" fullword ascii 
      $s4 = "\"%s\" help" fullword ascii
condition:
    any of them
}
...
rule FreeMilk_APT_Mal_2 {
   ...
   strings:
      ...
      $s2 = "runsample" fullword ascii 
      $s3 = "%s%02X%02X%02X%02X%02X%02X:" fullword ascii 
      $s4 = "win-%d.%d.%d-%d" fullword ascii 
condition:
    any of them
}
...
rule FreeMilk_APT_Mal_3 {
   ...
   strings:
      ...
      $s2 = "\\command\\start.exe" ascii 
      $s3 = ".bat;.com;.cmd;.exe" fullword ascii 
...
rule FreeMilk_APT_Mal_4 {
   ...
   strings:
      ...
      $s1 = "SOFTWARE\\Clients\\StartMenuInternet\\firefox.exe\\shell\\open\\command" fullword ascii 
