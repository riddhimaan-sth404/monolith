rule KINS_dropper {
	...
	strings:
		// Network protocol
		$n1 = "tid=%d&ta=%s-%x" fullword ascii
		$n2 = "fid=%d" fullword ascii
		$n3 = "%[^.].%[^(](%[^)])" fullword ascii
		// Injects
		$i0 = "%s [%s %d] 77 %s"
		$i01 = "Global\\%s%x"
		...
		$uac2 = "\\globalroot\\systemroot\\system32\\tasks\\" ascii 
		$uac3 = "<RunLevel>HighestAvailable</RunLevel>" ascii 
condition:
...
rule KINS_DLL_zeus {
	...
	strings:
		// Network protocol
		$n1 = "%BOTID%" fullword ascii
		$n2 = "%opensocks%" fullword ascii
		$n3 = "%openvnc%" fullword ascii
		$n4 = /Global\\(s|v)_ev/ fullword ascii
		// Crypted strings
