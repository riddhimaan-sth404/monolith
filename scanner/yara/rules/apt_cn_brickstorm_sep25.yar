...
rule WEBSHELL_G_APT_BackdoorWebshell_SLAYSTYLE_1 {
   ...
   strings:
      //$str1 = /String \w{1,10}=request\.getParameter\(\"\w{1,15}\"\);/ ascii
      $str1_alt = "=request.getParameter(\""
      $str2 = "=new String(java.util.Base64.getDecoder().decode(" ascii 
      //$str21 = /String\[\]\s\w{1,10}=\{\"\/bin\/sh\",\"-c\",\w{1,10}\+\"\s2>&1\"\};/ ascii
...
rule MAL_G_Dropper_BRICKSTEAL_2 {
   ...
   strings:
      ...
      $str1_alt = "(Class<?>) method.invoke(" ascii 
      $str2 = "(\"yv66vg" ascii
      $str3 = "request.getSession().getServletContext" ascii 
      $str4 = ".getClass().getDeclaredField(" ascii 
      $str5 = "new FilterDef();" ascii 
      $str6 = "new FilterMap();" ascii 
condition:
      all of them
}
