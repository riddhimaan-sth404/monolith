...
rule APT30_Sample_2 {
	...
	strings:
		$s0 = "ForZRLnkWordDlg.EXE" fullword ascii 
		$s1 = "ForZRLnkWordDlg Microsoft " fullword ascii 
		$s9 = "ForZRLnkWordDlg 1.0 " fullword ascii 
		$s11 = "ForZRLnkWordDlg" fullword ascii 
		$s12 = " (C) 2011" fullword ascii 
condition:
    any of them
}
...
rule APT30_Sample_5 {
	...
	strings:
		$s0 = "Version 4.7.3001" fullword ascii 
		$s1 = "Copyright (c) Microsoft Corporation 2004" fullword ascii 
		$s3 = "Microsoft(R) is a registered trademark of Microsoft Corporation in the U" ascii 
		$s7 = "msmsgs" fullword ascii 
...
rule APT30_Generic_I {
	...
	strings:
		$s0 = "Copyright 2012 Google Inc. All rights reserved." fullword ascii 
		$s1 = "(Prxy%c-%s:%u)" fullword ascii
		$s2 = "Google Inc." fullword ascii 
condition:
    any of them
}
...
rule APT30_Sample_10 {
	...
	strings:
		$s0 = "Version 4.7.3001" fullword ascii 
		$s1 = "Copyright (c) Microsoft Corporation 2004" fullword ascii 
		$s2 = "Microsoft(R) is a registered trademark of Microsoft Corporation in the U" ascii 
		$s3 = "!! Use Connect Method !!" fullword ascii
		$s4 = "(Prxy%c-%s:%u)" fullword ascii
		$s5 = "msmsgs" fullword ascii 
...
rule APT30_Sample_11 {
	...
	strings:
		...
		$s1 = "msofscan.exe" fullword ascii 
		$s2 = "Mozilla/4.0 (compatible; MSIE 5.0; Win32)" fullword ascii
		$s3 = "Microsoft? is a registered trademark of Microsoft Corporation." fullword ascii 
...
rule APT30_Sample_13 {
	meta:
		...
		id = "e5dd6bc9-9383-5d48-92df-709996373655"
	strings:
		$s0 = "msofscan.exe" fullword ascii 
		$s1 = "Microsoft? is a registered trademark of Microsoft Corporation." fullword ascii 
		$s2 = "Microsoft Office Word Plugin Scan" fullword ascii 
		$s3 = "? 2006 Microsoft Corporation.  All rights reserved." fullword ascii 
		$s4 = "msofscan" fullword ascii 
		$s6 = "2003 Microsoft Office system" fullword ascii 
condition:
    any of them
}
...
rule APT30_Sample_14 {
	meta:
		...
		id = "e5dd6bc9-9383-5d48-92df-709996373655"
	strings:
		$s0 = "AdobeReader.exe" fullword ascii 
		$s4 = "10.1.7.27" fullword ascii 
		$s5 = "Copyright 1984-2012 Adobe Systems Incorporated and its licensors. All ri" ascii 
		$s8 = "Adobe Reader" fullword ascii 
condition:
    any of them
}
...
rule APT30_Generic_D {
	...
	strings:
		$s0 = "Windows Security Service Feedback" fullword ascii 
		$s1 = "wssfmgr.exe" fullword ascii 
		$s2 = "\\rb.htm" ascii
		$s3 = "rb.htm" fullword ascii
		$s4 = "cook5" ascii
		$s5 = "5, 4, 2600, 0" fullword ascii 
condition:
    any of them
}
...
rule APT30_Generic_J {
	...
	strings:
		$s0 = "Launcher.EXE" fullword ascii 
		$s1 = "Symantec Security Technologies" fullword ascii 
		$s2 = "\\Symantec LiveUpdate.lnk" ascii
		$s3 = "Symantec Service Framework" fullword ascii 
		$s4 = "\\ccSvcHst.exe" ascii
		$s5 = "\\wssfmgr.exe" ascii
		$s6 = "Symantec Corporation" fullword ascii 
		$s7 = "\\5.1.0.29" ascii
		$s8 = "\\Engine" ascii
		$s9 = "Copyright (C) 2000-2010 Symantec Corporation. All rights reserved." fullword ascii 
...
rule APT30_Microfost {
	...
	strings:
		$s1 = "Copyright (c) 2007 Microfost All Rights Reserved" fullword ascii 
		$s2 = "Microfost" fullword ascii 
condition:
    any of them
}
...
rule APT30_Sample_33 {
	...
	strings:
		...
		$s7 = "Microsoft(R) is a registered trademark of Microsoft Corporation in the" ascii 
...
rule APT30_Generic_1 {
	...
	strings:
		$s0 = "%s\\%s.txt" fullword ascii
		$s1 = "\\ldsysinfo.txt"
		$s4 = "(Extended Wansung)" fullword ascii
		$s6 = "Computer Name:" fullword ascii
		$s7 = "%s %uKB %04u-%02u-%02u %02u:%02u" fullword ascii
		$s8 = "ASSAMESE" fullword ascii
		$s9 = "BELARUSIAN" fullword ascii
		$s10 = "(PR China)" fullword ascii
		$s14 = "(French)" fullword ascii
		$s15 = "AdvancedServer" fullword ascii
		$s16 = "DataCenterServer" fullword ascii
		$s18 = "(Finland)" fullword ascii
		$s19 = "%s %04u-%02u-%02u %02u:%02u" fullword ascii
		$s20 = "(Chile)" fullword ascii
condition:
    any of them
}
rule APT30_Generic_2 {
	...
	strings:
		$s0 = "%s\\%s\\KB985109.log" fullword ascii
		$s1 = "%s\\%s\\KB989109.log" fullword ascii
		$s2 = "Opera.exe" fullword ascii 
		$s3 = "%s:All online success on %u!" fullword ascii
		$s4 = "%s:list online success on %u!" fullword ascii
		$s5 = "%s:All online fail!" fullword ascii
		$s6 = "Copyright Opera Software 1995-" ascii 
		$s7 = "%s:list online fail!" fullword ascii
		$s8 = "OnlineTmp.txt" fullword ascii
		$s9 = "Opera Internet Browser" fullword ascii 
		$s12 = "Opera Software" fullword ascii 
		$s15 = "Check lan have done!!!" fullword ascii
		$s16 = "List End." fullword ascii
condition:
    any of them
}
...
rule APT30_Generic_3 {
	...
	strings:
		$s0 = "Acrobat.exe" fullword ascii 
		$s14 = "********************************" fullword ascii
		$s16 = "FFFF:>>>>>>>>>>>>>>>>>@" fullword ascii
condition:
    any of them
}
*/
...
rule APT30_Generic_4 {
	...
	strings:
		$s0 = "del NetEagle_Scout.bat" fullword ascii
		$s1 = "NetEagle_Scout.bat" fullword ascii
		$s2 = "\\visit.exe"
		$s3 = "\\System.exe"
		$s4 = "\\System.dat"
		$s5 = "\\ieupdate.exe"
		$s6 = "GOTO ERROR" fullword ascii
		$s7 = ":ERROR" fullword ascii
		$s9 = "IF EXIST " fullword ascii
		$s10 = "ioiocn" fullword ascii
		$s11 = "SetFileAttribute" fullword ascii
		$s12 = "le_0*^il" fullword ascii
		$s13 = "le_.*^il" fullword ascii
		$s14 = "le_-*^il" fullword ascii
condition:
    any of them
}
...
rule APT30_Generic_5 {
	...
	strings:
		$s0 = "regsvr32 /s \"%ProgramFiles%\\Norton360\\Engine\\5.1.0.29\\ashelper.dll\"" fullword ascii
		$s1 = "name=\"ftpserver.exe\"/>" fullword ascii
		$s2 = "LiveUpdate.EXE" fullword ascii 
		$s3 = "<description>FTP Explorer</description>" fullword ascii
		$s4 = "\\ashelper.dll"
		$s5 = "LiveUpdate" fullword ascii 
condition:
    any of them
}
...
rule APT30_Generic_6 {
	...
	strings:
		$s0 = "GetStar" fullword ascii
		$s1 = ".rdUaS" fullword ascii
		$s2 = "%sOTwp/&A\\L" fullword ascii
		$s3 = "a Encrt% Flash Disk" fullword ascii
		$s4 = "ypeAutoRuChec" fullword ascii
		$s5 = "NoDriveT" fullword ascii
condition:
    any of them
}
...
rule APT30_Generic_7 {
	...
	strings:
		$s1 = "Xjapor_*ata" fullword ascii
		$s2 = "Xjapor_o*ata" fullword ascii
		$s4 = "Ouopai" fullword ascii
condition:
    any of them
}
rule APT30_Generic_8 {
	...
	strings:
		$s0 = "Windows NT4.0" fullword ascii
		$s1 = "Windows NT3.51" fullword ascii
		$s2 = "%d;%d;%d;%ld;%ld;%ld;" fullword ascii
		$s3 = "%s %d.%d Build%d %s" fullword ascii
		$s4 = "MSAFD Tcpip [TCP/IP]" fullword ascii
		$s5 = "SQSRSS" fullword ascii
		$s8 = "WM_COMP" fullword ascii
		$s9 = "WM_MBU" fullword ascii
		$s11 = "WM_GRID" fullword ascii
		$s12 = "WM_RBU" fullword ascii
condition:
    any of them
}
...
rule APT30_Generic_9 {
	...
	strings:
		$s0 = "%s\\%s\\$NtRecDoc$" fullword ascii
		$s1 = "%s(%u)%s" fullword ascii
		$s2 = "http://%s%s%s" fullword ascii
		$s3 = "1.9.1.17" fullword ascii 
		$s4 = "(C)Firefox and Mozilla Developers, according to the MPL 1.1/GPL 2.0/LGPL" ascii 
condition:
    any of them
}
