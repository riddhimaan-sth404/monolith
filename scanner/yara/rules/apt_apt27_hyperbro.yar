...
rule HvS_APT27_HyperBro_Stage3 {
   ...
   strings:
      $s1 = "\\cmd.exe /A" ascii 
      $s2 = "vftrace.dll" fullword ascii 
      $s3 = "msmpeng.exe" fullword ascii 
      $s4 = "\\\\.\\pipe\\testpipe" fullword ascii 
      $s5 = "thumb.dat" fullword ascii 

      $g1 = "%s\\%d.exe" fullword ascii 
      $g2 = "https://%s:%d/api/v2/ajax" fullword ascii 
      $g3 = " -k networkservice" fullword ascii 
      $g4 = " -k localservice" fullword ascii 
...
rule HvS_APT27_HyperBro_Stage3_C2 {
   ...
   strings:
      $s1 = "api/v2/ajax" ascii 
      $s2 = "Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.116 Safari/537.36" ascii 
condition:
      all of them
}
