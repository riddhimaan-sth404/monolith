...
rule HKTL_RemoteCmd {
	...
	strings:
		$s1 = "RemoteCmd.exe" fullword ascii 
		$s2 = "\\Release\\RemoteCmd.pdb" ascii
		$s3 = "RemoteCmd [ComputerName] [Executable] [Param1] [Param2] ..." fullword ascii 
		$s4 = "http://{0}:65101/CommandEngine" fullword ascii 
...
rule HKTL_ChromePass {
	...
	strings:
		...
		$x2 = "Windows Protect folder for getting the encryption keys" ascii 
		$x3 = "Chrome User Data folder where the password file is stored" ascii 

		$s1 = "Opera Software\\Opera Stable\\Login Data" fullword ascii 
		$s2 = "Yandex\\YandexBrowser\\User Data\\Default\\Login Data" fullword ascii 
		$s3 = "Load the passwords from another Windows user or external drive: " fullword ascii 
		$s4 = "Windows Login Password:" fullword ascii 
		...
		$s6 = "Chrome Password Recovery" fullword ascii 
condition:
    any of them
}
