...
rule OpCloudHopper_Malware_2 {
   ...
   strings:
      ...
      $x2 = "ToolbarF.dll" fullword ascii 
      $x3 = ".?AVCKeyLoggerManager@@" fullword ascii
      $x4 = "GH0STCZH" ascii

      $s1 = "%%SystemRoot%%\\System32\\svchost.exe -k \"%s\"" fullword ascii
      $s2 = "rundll32.exe \"%s\", UnInstall /update %s" fullword ascii
      $s3 = "\\Release\\Loader.pdb" ascii
      $s4 = "%s\\%x.dll" fullword ascii 
      $s5 = "Mozilla/4.0 (compatible)" fullword ascii 
      $s6 = "\\syslog.dat" ascii 
      $s7 = "NSOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" fullword ascii 
...
rule OpCloudHopper_Malware_8 {
   ...
   strings:
      $s1 = "WSHELL32.dll" fullword ascii 
      $s2 = "operator \"\" " fullword ascii
      $s3 = "\" /t REG_SZ /d \"" fullword ascii
      $s4 = " /f /v \"" fullword ascii
      $s5 = "zok]\\\\\\ZZYYY666564444" fullword ascii
      $s6 = "AFX_DIALOG_LAYOUT" fullword ascii 
condition:
    any of them
}
...
rule OpCloudHopper_Malware_10 {
   ...
   strings:
      $x1 = "bakshell.EXE" fullword ascii 
      $s19 = "bakshell Applicazione MFC" fullword ascii 
