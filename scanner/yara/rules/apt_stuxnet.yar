...
rule Stuxnet_Malware_2 {
	...
	strings:
		$s1 = "\\SystemRoot\\System32\\hal.dll" ascii 
...
rule Stuxnet_Shortcut_to {
	...
	strings:
		$x1 = "\\\\.\\STORAGE#Volume#_??_USBSTOR#Disk&Ven_Kingston&Prod_DataTraveler_2.0&Rev_PMAP#5B6B098B97BE&0#{53f56307-b6bf-11d0-94f2-00a0c" ascii 
condition:
    any of them
}
...
rule Stuxnet_Malware_3 {
	...
	strings:
		$x1 = "SHELL32.DLL.ASLR." fullword ascii 

		$s1 = "~WTR4141.tmp" fullword ascii 
		$s2 = "~WTR4132.tmp" fullword ascii 
		$s3 = "totalcmd.exe" fullword ascii 
		$s4 = "wincmd.exe" fullword ascii 
		$s5 = "http://www.realtek.com0" fullword ascii
		$s6 = "{%08x-%08x-%08x-%08x}" fullword ascii 
condition:
    any of them
}
...
rule Stuxnet_Malware_4 {
	...
	strings:
		...
		$x2 = "MRxCls.sys" fullword ascii 
		$x3 = "MRXNET.Sys" fullword ascii 
condition:
    any of them
}
...
rule Stuxnet_maindll_decrypted_unpacked {
	...
	strings:
		$s1 = "%SystemRoot%\\system32\\Drivers\\mrxsmb.sys;%SystemRoot%\\system32\\Drivers\\*.sys" fullword ascii 
		$s2 = "<Actions Context=\"%s\"><Exec><Command>%s</Command><Arguments>%s,#%u</Arguments></Exec></Actions>" fullword ascii
		$s3 = "%SystemRoot%\\inf\\oem7A.PNF" fullword ascii 
		$s4 = "%SystemRoot%\\inf\\mdmcpq3.PNF" fullword ascii 
		$s5 = "%SystemRoot%\\inf\\oem6C.PNF" fullword ascii 
		$s6 = "@abf varbinary(4096) EXEC @hr = sp_OACreate 'ADODB.Stream', @aods OUT IF @hr <> 0 GOTO endq EXEC @hr = sp_OASetProperty @" ascii 
		$s7 = "STORAGE#Volume#1&19f7e59c&0&" fullword ascii 
...
rule Stuxnet_s7hkimdb {
	...
	strings:
		$x1 = "S7HKIMDX.DLL" fullword ascii 
