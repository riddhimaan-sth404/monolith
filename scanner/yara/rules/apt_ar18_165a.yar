...
rule APT_NK_AR18_165A_1 {
   ...
   strings:
      $s1 = "netsh.exe advfirewall firewall add rule name=\"PortOpenning\" dir=in protocol=tcp localport=%d action=allow enable=yes" fullword ascii
      $s2 = "netsh.exe firewall add portopening TCP %d \"PortOpenning\" enable" fullword ascii
condition:
    any of them
}
