...
rule poison_ivy_softcell {
   ...
   strings:
      ...
      $s1 = "Cannot create folder %sDCRC failed in the encrypted file %s. Corrupt file or wrong password." fullword ascii 
      $s2 = "Extracting files to %s folder$Extracting files to temporary folder" fullword ascii 
      $s3 = "&Enter password for the encrypted file:" fullword ascii 
      $s4 = "start \"\" \"%CD%\\mcoemcpy.exe\"" fullword ascii
      $s5 = "setup.bat" fullword ascii
      $s6 = "ErroraErrors encountered while performing the operation" fullword ascii 
      $s7 = "Please download a fresh copy and retry the installation" fullword ascii 
      $s8 = "antivir.dat" fullword ascii
      $s9 = "The required volume is absent2The archive is either in unknown format or damaged" fullword ascii 
      $s10 = "=Total path and file name length must not exceed %d characters" fullword ascii 
      $s11 = "Please close all applications, reboot Windows and restart this installation\\Some installation files are corrupt." fullword ascii 
...
rule mangzamel_softcell {
   ...
   strings:
      ...
      $s2 = "spoolsvs.exe" fullword ascii 
...
rule mimikatz_utility_softcell {
   ...
   strings:
      ...
      $s1 = "livessp.dll" fullword ascii 
      $s2 = "\\system32\\tapi32.dll" fullword ascii 
      $s3 = " * Process Token : " fullword ascii 
      $s4 = "lsadump" fullword ascii 
      $s5 = "-nl - skip lsa dump..." fullword ascii 
      $s6 = "lsadump::sam" fullword ascii 
      $s7 = "lsadump::lsa" fullword ascii 
      $s8 = "* NL$IterCount %u, %u real iter(s)" fullword ascii 
      $s9 = "* Iter to def (%d)" fullword ascii 
      $s10 = " * Thread Token  : " fullword ascii 
      $s11 = " * RootKey  : " fullword ascii 
      $s12 = "lsadump::cache" fullword ascii 
      $s13 = "sekurlsa::logonpasswords" fullword ascii 
      $s14 = "(commandline) # %s" fullword ascii 
      $s15 = ">>> %s of '%s' module failed : %08x" fullword ascii 
      $s16 = "UndefinedLogonType" fullword ascii 
      $s17 = " * Username : %wZ" fullword ascii 
      $s18 = "logonPasswords" fullword ascii 
      $s19 = "privilege::debug" fullword ascii 
      $s20 = "token::elevate" fullword ascii 
...
rule sfx_winrar_plugx {
   ...
   strings:
      ...
      $s1 = "Cannot create folder %sDCRC failed in the encrypted file %s. Corrupt file or wrong password." fullword ascii 
      $s2 = "Wrong password for %s5Write error in the file %s. Probably the disk is full" fullword ascii 
      $s3 = "mcutil.dll" fullword ascii
      $s4 = "Unexpected end of archiveThe file \"%s\" header is corrupt%The archive comment header is corrupt" fullword ascii
      $s5 = "mcoemcpy.exe" fullword ascii
      $s6 = "Extracting files to %s folder$Extracting files to temporary folder" fullword ascii 
      $s7 = "&Enter password for the encrypted file:" fullword ascii 
      $s8 = "start \"\" \"%CD%\\mcoemcpy.exe\"" fullword ascii
      $s9 = "setup.bat" fullword ascii
      $s10 = "ErroraErrors encountered while performing the operation" fullword ascii 
      $s11 = "Please download a fresh copy and retry the installation" fullword ascii 
      $s12 = "antivir.dat" fullword ascii
      $s13 = "The required volume is absent2The archive is either in unknown format or damaged" fullword ascii 
      $s14 = "=Total path and file name length must not exceed %d characters" fullword ascii 
      $s15 = "Please close all applications, reboot Windows and restart this installation\\Some installation files are corrupt." fullword ascii 
      $s16 = "folder is not accessiblelSome files could not be created." fullword ascii 
      $s17 = "Packed data CRC failed in %s" fullword ascii 
      $s18 = "DDTTDTTDTTDTTDTTDTTDTTDTTDTQ" fullword ascii
      $s19 = "File close error" fullword ascii 
      $s20 = "CRC failed in %s" fullword ascii 
