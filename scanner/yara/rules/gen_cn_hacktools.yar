...
rule WAF_Bypass {
	...
	strings:
		$s1 = "Email: blacksplitn@gmail.com" fullword ascii 
		$s2 = "User-Agent:" fullword ascii 
		$s3 = "Send Failed.in RemoteThread" fullword ascii
		$s4 = "www.example.com" fullword ascii 
...
rule MarathonTool {
	...
	strings:
		...
		$s18 = "SELECT UNICODE(SUBSTRING((system_user),{0},1))" fullword ascii 
condition:
    any of them
}
...
rule sekurlsa {
	...
	strings:
		$s1 = "Bienvenue dans un processus distant" fullword ascii 
		$s2 = "Format d'appel invalide : addLogonSession [idSecAppHigh] idSecAppLow Utilisateur" ascii 
		$s3 = "SECURITY\\Policy\\Secrets" fullword ascii 
		$s4 = "Injection de donn" fullword ascii 
condition:
    any of them
}
...
rule DTools2_02_DTools {
	...
	strings:
		...
		$s1 = "TSETPASSWORDFORM" fullword ascii 
		$s2 = "TGETNTUSERNAMEFORM" fullword ascii 
		$s3 = "TPORTFORM" fullword ascii 
...
rule dll_PacketX {
	...
	strings:
		$s9 = "[Failed to load winpcap packet.dll." ascii 
		$s10 = "PacketX Version" ascii 
condition:
    any of them
}
...
rule CookieTools {
	...
	strings:
		...
		$s2 = "No data to read.$Can not bind in port range (%d - %d)" fullword ascii 
		$s3 = "Connection Closed Gracefully.;Could not bind socket. Address and port are alread" ascii 
...
rule IDTools_For_WinXP_IdtTool {
	...
	strings:
		...
		$s4 = "Idt Tool bY tMd[CsP]" fullword ascii 
...
rule Cmdshell32 {
	...
	strings:
		$s1 = "cmdshell.exe" fullword ascii 
		$s2 = "cmdshell" fullword ascii
		$s3 = "[Root@CmdShell ~]#" fullword ascii 
condition:
    any of them
}
...
rule x64_klock {
	...
	strings:
		...
		$s3 = "Erreur : le bureau courant (" ascii 
		$s4 = "klock de mimikatz pour Windows" fullword ascii 
condition:
    any of them
}
...
rule Dos_Down32 {
	...
	strings:
		$s2 = "C:\\Windows\\Temp\\Cmd.txt" fullword ascii 
		$s6 = "down.exe" fullword ascii 
...
rule MarathonTool_2 {
	...
	strings:
		$s3 = "http://localhost/retomysql/pista.aspx?id_pista=1" fullword ascii 
		$s6 = "SELECT ASCII(SUBSTR(username,{0},1)) FROM USER_USERS" fullword ascii 
		$s17 = "/Blind SQL injection tool based in heavy queries" fullword ascii
condition:
    any of them
}
...
rule CN_Tools_PcShare {
	...
	strings:
		$s0 = "title=%s%s-%s;id=%s;hwnd=%d;mainhwnd=%d;mainprocess=%d;cmd=%d;" fullword ascii 
		$s1 = "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.0; .NET CLR 1.1.4322)" fullword ascii 
		$s2 = "http://www.pcshares.cn/pcshare200/lostpass.asp" fullword ascii 
		$s5 = "port=%s;name=%s;pass=%s;" fullword ascii 
		$s16 = "%s\\ini\\*.dat" fullword ascii 
		$s17 = "pcinit.exe" fullword ascii 
		$s18 = "http://www.pcshare.cn" fullword ascii 
condition:
    any of them
}
...
rule Dll_LoadEx {
	...
	strings:
		$s0 = "WiNrOOt@126.com" fullword ascii 
		$s1 = "Dll_LoadEx.EXE" fullword ascii 
		$s3 = "You Already Loaded This DLL ! :(" ascii
		$s10 = "Dll_LoadEx Microsoft " fullword ascii 
		$s17 = "Can't Load This Dll ! :(" ascii
		$s18 = "WiNrOOt" fullword ascii 
		$s20 = " Dll_LoadEx(&A)..." fullword ascii 
condition:
    any of them
}
...
rule SwitchSniffer {
	...
	strings:
		$s0 = "NextSecurity.NET" fullword ascii 
		$s2 = "SwitchSniffer Setup" fullword ascii 
condition:
...
rule WebCrack4_RouterPasswordCracking {
	...
	strings:
		...
		$s20 = "List count out of bounds (%d)+Operation not allowed on sorted string list%String" ascii 
condition:
    any of them
}
...
rule hscan_gui {
	...
	strings:
		$s0 = "Hscan.EXE" fullword ascii 
		$s1 = "RestTool.EXE" fullword ascii
		$s3 = "Hscan Application " fullword ascii 
condition:
    any of them
}
...
rule HKTL_CN_dat_xpf {
	...
	strings:
		...
		$s2 = "\\Device\\XScanPF" ascii 
		$s3 = "\\DosDevices\\XScanPF" ascii 
condition:
    any of them
}
...
rule Arp_EMP_v1_0 {
	...
	strings:
		$s0 = "Arp EMP v1.0.exe" fullword ascii 
condition:
    any of them
}
...
rule CN_Tools_Shiell {
	...
	strings:
		...
		$s2 = "C:\\Windows\\System32\\Shiell.exe" fullword ascii 
		$s3 = "Shift shell.exe" fullword ascii 
		$s4 = "\" /v debugger /t REG_SZ /d \"" fullword ascii
condition:
    any of them
}
...
rule cndcom_cndcom {
	...
	strings:
		...
		$s7 = "\\C$\\123456111111111111111.doc" ascii 
...
rule IsDebug_V1_4 {
	...
	strings:
		...
		$s1 = "SV Dumper V1.0" fullword ascii 
...
rule HTTPSCANNER {
	...
	strings:
		$s1 = "HttpScanner.exe" fullword ascii 
		$s2 = "HttpScanner" fullword ascii 
condition:
    any of them
}
...
rule Dos_netstat {
	...
	strings:
		...
		$s1 = "Retransmission Timeout Algorithm    = unknown (%1!u!)" fullword ascii /* Goodware String - occured 2 times */
		$s2 = "Administrative Status  = %1!u!" fullword ascii /* Goodware String - occured 2 times */
		$s3 = "Packet Too Big            %1!-10u!  %2!-10u!" fullword ascii /* Goodware String - occured 2 times */
condition:
    any of them
}
...
rule MSSqlPass {
	...
	strings:
		$s0 = "Reveals the passwords stored in the Registry by Enterprise Manager of SQL Server" ascii 
		$s1 = "empv.exe" fullword ascii 
		$s2 = "Enterprise Manager PassView" fullword ascii 
condition:
    any of them
}
...
rule WSockExpert {
	...
	strings:
		...
		$s3 = "'%s' is not a valid time!'%s' is not a valid date and time" fullword ascii 
...
rule Dos_Down64 {
	...
	strings:
		$s1 = "C:\\Windows\\Temp\\Down.txt" fullword ascii 
		$s2 = "C:\\Windows\\Temp\\Cmd.txt" fullword ascii 
		$s3 = "C:\\Windows\\Temp\\" ascii 
		$s4 = "ProcessXElement" fullword ascii
		$s8 = "down.exe" fullword ascii 
...
rule Tools_unknown {
	...
	strings:
		$s1 = "No data to read.$Can not bind in port range (%d - %d)" fullword ascii 
		...
		$s4 = "Failed to clear tab control Failed to delete tab at index %d\"Failed to retrieve" ascii
...
rule PLUGIN_AJunk {
	...
	strings:
		...
		$s2 = "AJunk.DLL" fullword ascii 
		$s3 = "AJunk Dynamic Link Library" fullword ascii 
condition:
    any of them
}
...
rule IISPutScanner {
	...
	strings:
		...
		$s23 = "BBABORT" fullword ascii 
		$s25 = "BBCANCEL" fullword ascii 
		$s26 = "BBCLOSE" fullword ascii 
		$s27 = "BBHELP" fullword ascii 
		$s28 = "BBIGNORE" fullword ascii 
		$s29 = "PREVIEWGLYPH" fullword ascii 
		$s30 = "DLGTEMPLATE" fullword ascii 
		$s31 = "TABOUTBOX" fullword ascii 
		$s32 = "TFORM1" fullword ascii 
		$s33 = "MAINICON" fullword ascii 
condition:
    any of them
}
...
rule x_way2_5_X_way {
	...
	strings:
		$s0 = "TTFTPSERVERFRM" fullword ascii 
		$s1 = "TPORTSCANSETFRM" fullword ascii 
		$s2 = "TIISSHELLFRM" fullword ascii 
		$s3 = "TADVSCANSETFRM" fullword ascii 
		$s4 = "ntwdblib.dll" fullword ascii
		$s5 = "TSNIFFERFRM" fullword ascii 
		$s6 = "TCRACKSETFRM" fullword ascii 
		$s7 = "TCRACKFRM" fullword ascii 
...
rule Sword1_5 {
	...
	strings:
		$s3 = "http://www.ip138.com/ip2city.asp" fullword ascii 
		$s4 = "http://www.md5decrypter.co.uk/feed/api.aspx?" fullword ascii 
		$s6 = "ListBox_Command" fullword ascii 
		$s13 = "md=7fef6171469e80d32c0559f88b377245&submit=MD5+Crack" fullword ascii 
		$s18 = "\\Set.ini" ascii 
		$s19 = "OpenFileDialog1" fullword ascii 
		$s20 = " (*.txt)|*.txt" fullword ascii 
condition:
    any of them
}
...
rule Tools_scan {
	...
	strings:
		$s2 = "Shanlu Studio" fullword ascii 
...
rule datPcShare {
	...
	strings:
		$s1 = "PcShare.EXE" fullword ascii 
		$s2 = "MZKERNEL32.DLL" fullword ascii
		$s3 = "PcShare" fullword ascii 
		$s4 = "QQ:4564405" fullword ascii 
condition:
    any of them
}
...
rule Pc_xai {
	...
	strings:
		$s1 = "Powered by CoolDiyer @ C.Rufus Security Team 05/19/2008  http://www.xcodez.com/" fullword ascii 
		$s2 = "%SystemRoot%\\System32\\" ascii
		$s3 = "%APPDATA%\\" ascii
		$s4 = "---- C.Rufus Security Team ----" fullword ascii 
		$s5 = "www.snzzkz.com" fullword ascii 
...
rule Radmin_Hash {
	...
	strings:
		...
		$s3 = "Radmin, Remote Administrator" fullword ascii 
		$s4 = "Radmin 3.0 Hash " fullword ascii 
		$s5 = "HASH1.0" fullword ascii 
condition:
    any of them
}
...
rule OSEditor {
	...
	strings:
		$s1 = "OSEditor.exe" fullword ascii 
		$s2 = "netsafe" ascii 
		$s3 = "OSC Editor" fullword ascii 
		$s4 = "GIF89" ascii
		$s5 = "Unlock" ascii
condition:
    any of them
...
rule GoodToolset_ms11011 {
	...
	strings:
		...
		$s3 = "Not supported." fullword ascii /* Goodware String - occured 3 times */
		$s4 = "SystemDefaultEUDCFont" fullword ascii /* Goodware String - occured 18 times */
condition:
    any of them
}
...
rule x64_KiwiCmd {
	...
	strings:
		$s1 = "Process Ok, Memory Ok, resuming process :)" fullword ascii 
		$s2 = "Kiwi Cmd no-gpo" fullword ascii 
		$s3 = "KiwiAndCMD" fullword ascii 
condition:
    any of them
}
...
rule CookieTools2 {
	...
	strings:
		$s1 = "www.gxgl.com&www.gxgl.net" fullword ascii 
...
rule GoodToolset_pr {
	...
	strings:
		...
		$s6 = "root\\MicrosoftIISv2" fullword ascii 
condition:
    any of them
}
...
rule CmdShell64 {
	...
	strings:
		$s1 = "C:\\Windows\\System32\\JAVASYS.EXE" fullword ascii 
		$s2 = "ServiceCmdShell" fullword ascii
		$s3 = "<!-- If your application is designed to work with Windows 8.1, uncomment the fol" ascii
		$s4 = "ServiceSystemShell" fullword ascii 
		$s5 = "[Root@CmdShell ~]#" fullword ascii 
		$s6 = "Hello Man 2015 !" fullword ascii 
...
rule HKTL_Unknown_CN_Generate {
	...
	strings:
		$s1 = "C:\\TEMP\\" ascii
		$s2 = "Connection Closed Gracefully.;Could not bind socket. Address and port are alread" ascii 
...
rule Pc_rejoice {
	...
	strings:
		...
		$s4 = "No data to read.$Can not bind in port range (%d - %d)" fullword ascii 
...
rule unknown2 {
	...
	strings:
		$s1 = "http://md5.com.cn/index.php/md5reverse/index/md/" ascii 
		$s2 = "http://www.md5decrypter.co.uk/feed/api.aspx?" ascii 
		$s3 = "http://www.md5.com.cn" fullword ascii 
		$s4 = "1.5.exe" fullword ascii 
		$s5 = "\\Set.ini" ascii 
		$s6 = "OpenFileDialog1" fullword ascii 
		$s7 = " (*.txt)|*.txt" fullword ascii 
condition:
    any of them
}
...
rule SQLTools {
	...
	strings:
		$s1 = "DBN_POST" fullword ascii 
		$s2 = "LOADER ERROR" fullword ascii
		$s3 = "www.1285.net" fullword ascii 
		$s4 = "TUPFILEFORM" fullword ascii 
		$s5 = "DBN_DELETE" fullword ascii 
		$s6 = "DBINSERT" fullword ascii 
		$s7 = "Copyright (C) Kibosoft Corp. 2001-2006" fullword ascii 
condition:
    any of them
}
...
rule kappfree {
	...
	strings:
		...
		$s3 = "kappfree de mimikatz pour Windows (anti AppLocker)" fullword ascii 
condition:
    any of them
}
...
rule KiwiTaskmgr_2 {
	...
	strings:
		$s1 = "Process Ok, Memory Ok, resuming process :)" fullword ascii 
		$s2 = "Kiwi Taskmgr no-gpo" fullword ascii 
		$s3 = "KiwiAndTaskMgr" fullword ascii 
condition:
    any of them
}
...
rule kappfree_2 {
	...
	strings:
		...
		$s2 = "kappfree de mimikatz pour Windows (anti AppLocker)" fullword ascii 
		$s3 = "' introuvable !" fullword ascii 
		$s4 = "kiwi\\mimikatz" fullword ascii 
condition:
    any of them
}
...
rule Win32_klock {
	...
	strings:
		...
		$s2 = "Erreur : impossible de basculer le bureau ; SwitchDesktop : " fullword ascii 
		$s3 = "klock de mimikatz pour Windows" fullword ascii 
condition:
    any of them
}
...
rule epathobj_exp64 {
	...
	strings:
		...
		$s6 = "- inconsistent onexit begin-end variables" fullword ascii /* Goodware String - occured 96 times */
condition:
    any of them
}
...
rule kelloworld_2 {
	...
	strings:
		...
		$s3 = "kelloworld de mimikatz pour Windows" fullword ascii 
condition:
    any of them
}
...
rule _Project1_Generate_rejoice {
	...
	strings:
		...
		$s5 = "%s is not a valid IP address." fullword ascii 
...
rule kiwi_tools_gentil_kiwi {
	...
	strings:
		$s1 = "mimikatz" fullword ascii 
		$s2 = "Copyright (C) 2012 Gentil Kiwi" fullword ascii 
		$s3 = "Gentil Kiwi" fullword ascii 
condition:
    any of them
}
