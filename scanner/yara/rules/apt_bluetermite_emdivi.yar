...
rule Emdivi_SFX {
	...
	strings:
		...
		$s1 = "&Enter password for the encrypted file:" fullword ascii 
...
rule Emdivi_Gen1 {
	...
	strings:
		$x1 = "wmic nteventlog where filename=\"SecEvent\" call cleareventlog" fullword ascii
		$x2 = "del %Temp%\\*.exe %Temp%\\*.dll %Temp%\\*.bat %Temp%\\*.ps1 %Temp%\\*.cmd /f /q" fullword ascii 
		$x3 = "userControl-v80.exe" fullword ascii

		$s1 = "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; SV1; .NET CLR 2.0.50727.42)" fullword ascii 
		$s2 = "http://www.msftncsi.com" fullword ascii 
		$s3 = "net use | find /i \"c$\"" fullword ascii
		$s4 = " /del /y & " fullword ascii 
		$s5 = "\\auto.cfg" ascii 
		$s6 = "/ncsi.txt" fullword ascii 
		$s7 = "Dcmd /c" fullword ascii 
		$s8 = "/PROXY" fullword ascii 
condition:
    any of them
}
