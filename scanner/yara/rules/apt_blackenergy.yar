...
rule BlackEnergy_BE_2 {
   ...
   strings:
      ...
      $s1 = "WindowsSysUtility - Unicode" fullword ascii 
      $s2 = "msiexec.exe" fullword ascii 
...
rule BlackEnergy_KillDisk_1 {
	...
	strings:
		...
		$s1 = "system32\\icacls.exe" fullword ascii 
		$s2 = "/c del /F /S /Q %c:\\*.*" fullword ascii
		$s3 = "shutdown /r /t %d" fullword ascii
		$s4 = "/C /Q /grant " fullword ascii 
		$s5 = "%08X.tmp" fullword ascii
		$s6 = "/c format %c: /Y /X /FS:NTFS" fullword ascii
		$s7 = "/c format %c: /Y /Q" fullword ascii
		$s8 = "taskhost.exe" fullword ascii /* Goodware String - occured 1 times */
		$s9 = "shutdown.exe" fullword ascii /* Goodware String - occured 1 times */
condition:
    any of them
}
...
rule BlackEnergy_KillDisk_2 {
	...
	strings:
		...
		$s2 = ".exe.sys.drv.doc.docx.xls.xlsx.mdb.ppt.pptx.xml.jpg.jpeg.ini.inf.ttf" ascii 
		$s3 = "%ls_%ls_%ls_%d.~tmp" fullword ascii 
condition:
    any of them
}
...
rule BlackEnergy_Driver_USBMDM {
	...
	strings:
		$s1 = "USB MDM Driver" fullword ascii 
...
rule BlackEnergy_Driver_AMDIDE {
	...
	strings:
		$s1 = " AMD IDE driver" fullword ascii 
		$s2 = "SessionEnv" fullword ascii 
		$s3 = "\\DosDevices\\{C9059FFF-1C49-4445-83E8-" ascii 
		$s4 = "\\Device\\{C9059FFF-1C49-4445-83E8-" ascii 
condition:
    any of them
}
