...
rule Industroyer_Malware_1 {
   ...
   strings:
      ...
      $s2 = "SYSTEM\\CurrentControlSet\\Services\\%ls" fullword ascii 
      $s3 = "SYS_BASCON.COM" fullword ascii 
      $s4 = "*.pcmt" fullword ascii 
      $s5 = "*.pcmi" fullword ascii 
...
rule Industroyer_Malware_2 {
   ...
   strings:
      $x1 = "sc create %ls type= own start= auto error= ignore binpath= \"%ls\" displayname= \"%ls\"" fullword ascii
      $x2 = "10.15.1.69:3128" fullword ascii 

      $s1 = "Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; InfoPath.1)" fullword ascii 
      $s2 = "/c sc stop %s" fullword ascii 
      $s3 = "sc start %ls" fullword ascii 
      $s4 = "93.115.27.57" fullword ascii 
      $s5 = "5.39.218.152" fullword ascii 
      $s6 = "tierexe" fullword ascii 
      $s7 = "comsys" fullword ascii 
      $s8 = "195.16.88.6" fullword ascii 
      $s9 = "TieringService" fullword ascii 

      $a1 = "TEMP\x00\x00DEF" fullword ascii 
      $a2 = "TEMP\x00\x00DEF-C" fullword ascii 
      $a3 = "TEMP\x00\x00DEF-WS" fullword ascii 
      $a4 = "TEMP\x00\x00DEF-EP" fullword ascii 
      $a5 = "TEMP\x00\x00DC-2-TEMP" fullword ascii 
      $a6 = "TEMP\x00\x00DC-2" fullword ascii 
      $a7 = "TEMP\x00\x00CES-McA-TEMP" fullword ascii 
      $a8 = "TEMP\x00\x00SRV_WSUS" fullword ascii 
      $a9 = "TEMP\x00\x00SRV_DC-2" fullword ascii 
      $a10 = "TEMP\x00\x00SCE-WSUS01" fullword ascii 
condition:
    any of them
}
...
rule Industroyer_Portscan_3 {
   ...
   strings:
      ...
      /* Decompressed File */
      $d1 = "Error params Arguments!!!" fullword ascii 
      $d2 = "^(.+?.exe).*\\s+-ip\\s*=\\s*(.+)\\s+-ports\\s*=\\s*(.+)$" fullword ascii 
      $d3 = "Exhample:App.exe -ip= 127.0.0.1-100," fullword ascii 
      $d4 = "Error IP Range %ls - %ls" fullword ascii 
      $d5 = "Can't closesocket." fullword ascii 
condition:
    any of them
}
