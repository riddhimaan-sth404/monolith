...
rule FiveEyes_QUERTY_Malwaresig_20123_sys {
	...
	strings:
		...
		$s1 = "kbdclass.sys" fullword ascii 
...
rule FiveEyes_QUERTY_Malwaresig_20120_dll {
	...
	strings:
		$s0 = "\\QwLog_%d-%02d-%02d-%02d%02d%02d.txt" ascii 
		$s1 = "\\QwLog_%d-%02d-%02d-%02d%02d%02d.xml" ascii 
		$s2 = "Failed to send the EQwerty_driverStatusCommand to the implant." fullword ascii
		$s3 = "- Log Used (number of windows) - %d" fullword ascii 
		$s4 = "- Log Limit (number of windows) - %d" fullword ascii 
		$s5 = "Process or User Default Language" fullword ascii 
		$s6 = "Windows 98/Me, Windows NT 4.0 and later: Vietnamese" fullword ascii 
		$s7 = "- Logging of keystrokes is switched ON" fullword ascii 
		$s8 = "- Logging of keystrokes is switched OFF" fullword ascii 
		$s9 = "Qwerty is currently logging active windows with titles containing the fo" ascii 
		$s10 = "Windows 95, Windows NT 4.0 only: Korean (Johab)" fullword ascii 
		$s11 = "FAILED to get Qwerty Status" fullword ascii 
		$s12 = "- Successfully retrieved Log from Implant." fullword ascii 
		$s13 = "- Logging of all Windows is toggled ON" fullword ascii 
		$s14 = "- Logging of all Windows is toggled OFF" fullword ascii 
		$s15 = "Qwerty FAILED to retrieve window list." fullword ascii 
		$s16 = "- UNSUCCESSFUL Log Retrieval from Implant." fullword ascii 
		$s17 = "The implant failed to return a valid status" fullword ascii
		$s18 = "- Log files were NOT generated!" fullword ascii 
		$s19 = "Windows 2000/XP: Armenian. This is Unicode only." fullword ascii 
		$s20 = "- This machine is using a PS/2 Keyboard - Continue on using QWERTY" fullword ascii 
condition:
		10 of them
}
