...
rule NotPetya_Ransomware_Jun17 {
   ...
   strings:
      $x1 = "Ooops, your important files are encrypted." fullword ascii
      $x2 = "process call create \"C:\\Windows\\System32\\rundll32.exe \\\"C:\\Windows\\%s\\\" #1 " fullword ascii
      $x3 = "-d C:\\Windows\\System32\\rundll32.exe \"C:\\Windows\\%s\",#1 " fullword ascii
      $x4 = "Send your Bitcoin wallet ID and personal installation key to e-mail " fullword ascii 
      $x5 = "fsutil usn deletejournal /D %c:" fullword ascii 
      ...
      $s1 = "%s /node:\"%ws\" /user:\"%ws\" /password:\"%ws\" " fullword ascii
      $s4 = "\\\\.\\pipe\\%ws" fullword ascii 
      $s5 = "schtasks %ws/Create /SC once /TN \"\" /TR \"%ws\" /ST %02d:%02d" fullword ascii
      $s6 = "u%s \\\\%s -accepteula -s " fullword ascii 
      $s7 = "dllhost.dat" fullword ascii 
condition:
    any of them
}