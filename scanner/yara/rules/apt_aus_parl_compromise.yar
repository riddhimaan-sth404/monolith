...
rule HKTL_LazyCat_LogEraser {
   ...
   strings:
      $x1 = "LazyCat.dll" ascii fullword
      $x2 = ".local_privilege_escalation.rotten_potato" ascii 
      $x3 = "LazyCat.Extension" ascii 
      $x4 = " MEOWof" ascii 
      $x5 = "VirtualSite: {0}, Address: {1:X16}, Name: {2}, Handle: {3:X16}, LogPath: {4}" fullword ascii 

      $s1 = "LazyCat" fullword ascii 
...
rule HKTL_PowerKatz_Feb19_1 {
   ...
   strings:
      $x1 = "Powerkatz32" ascii fullword
      $x2 = "Powerkatz64" ascii 

      $s1 = "GetData: not found taskName" fullword ascii 
      $s2 = "GetRes Ex:" fullword ascii 
condition:
      1 of ($x*) and 1 of ($s*)
}
...
rule HKTL_Unknown_Feb19_1 {
   ...
   strings:
      $x1 = "not a valid timeout format!" ascii fullword
      $x2 = "host can not be empty!" ascii fullword
      $x3 = "not a valid port format!" ascii fullword
      $x4 = "{0} - {1} TTL={2} time={3}" ascii fullword
      $x5 = "ping count is not a correct format!" ascii fullword

      $s1 = "The result is too large,program store to '{0}'.Please download it manully." fullword ascii 
      $s2 = "C:\\Windows\\temp\\" ascii 
condition:
      1 of ($x*) or 2 of them
}
