...
rule TempRacer {
	...
	strings:
		...
		$s2 = "[+] Injecting into " fullword ascii 
		$s3 = "net localgroup administrators alex /add" fullword ascii 
		$s4 = "[+] File: {0} renamed to {1}" fullword ascii 
		$s5 = "[+] Blocking " fullword ascii 
condition:
    any of them
}
