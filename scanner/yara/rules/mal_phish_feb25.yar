...
rule MAL_PHISH_Final_Payload_Feb25 {
   ...
   strings:
      $s1 = "%lu: %s %s" ascii 
      $s2 = "(Direct Inbound)" ascii 
      $s3 = "(Primary Domain)" ascii 
      $s4 = "(Forest Tree Root" ascii 
      $s5 = "(Native Mode)" ascii 
      $s6 = "(In Forest)" ascii 
      $s7 = "(None)" ascii 
condition:
      all of them
...
rule SUSP_Sysinternals_Desktops_Anomaly_Feb25 {
   ...
   strings:
      $s1 = "Software\\Sysinternals\\Desktops" ascii fullword
      $s2 = "Sysinternals Desktops" ascii fullword
      $s3 = "http://www.sysinternals.com" ascii fullword
condition:
    any of them
}
