...
rule Fireball_de_svr {
   ...
   strings:
      ...
      $s2 = "rundll32.exe \"%s\",%s" fullword ascii
      $s3 = "http://d12zpbetgs1pco.cloudfront.net/Weatherapi/shell" fullword ascii 
...
rule Fireball_lancer {
   ...
   strings:
      ...
      $x2 = "lanceruse.dat" fullword ascii 

      $s1 = "Lancer.dll" fullword ascii
      $s2 = "RunDll32.exe \"" fullword ascii
      $s3 = "Micr.dll" fullword ascii 
      $s4 = "AG64.dll" fullword ascii 
...
rule QQBrowser {
   ...
   strings:
      ...
      $s2 = ".Downloader.dll" fullword ascii 
      $s3 = "Software\\Chromium\\BrowserCrashDumpAttempts" fullword ascii 
      $s4 = "QQBrowser_Broker.exe" fullword ascii 
condition:
    any of them
}
...
rule chrome_elf {
   ...
   strings:
      $x2 = "schtasks /Create /SC HOURLY /MO %d /ST 00:%02d:00 /TN \"%s\" /TR \"%s\" /RU \"SYSTEM\"" fullword ascii
      $s6 = "aHR0cDovL2R2Mm0xdXVtbnNndHUuY2xvdWRmcm9udC5uZXQvdjQvZ3RnLyVzP2FjdGlvbj12aXNpdC5jaGVsZi5pbnN0YWxs" fullword ascii /* encoded string 'http://dv2m1uumnsgtu.cloudfront.net/v4/gtg/%s?action=visit.chelf.install' */
      $s7 = "QueryInterface call failed for IExecAction: %x" fullword ascii
      $s10 = "%s %s,Rundll32_Do %s" fullword ascii 
...
rule Fireball_regkey {
   ...
   strings:
      ...
      $s2 = "ScreenShot" fullword ascii 
      $s3 = "WINMAIN" fullword ascii 
condition:
    any of them
}
...
rule Fireball_winsap {
   ...
   strings:
      $s1 = "aHR0cDovL2" ascii /* encoded string 'http://d3i1asoswufp5k.cloudfront.net/v4/gtg/%s?action=visit.winsap.work&update3=version,%s' */
      $s2 = "%s\\svchost.exe -k %s" fullword ascii 
      $s3 = "\\SETUP.dll" ascii 
      $s4 = "WinSAP.dll" fullword ascii
      $s5 = "Error %u in WinHttpQueryDataAvailable." fullword ascii
      $s6 = "UPDATE OVERWRITE" fullword ascii 
condition:
    any of them
}
...
rule Fireball_archer {
   ...
   strings:
      ...
      $s3 = "%s_%08X" fullword ascii 
      $s4 = "d\\\\.\\PhysicalDrive%d" fullword ascii 
condition:
    any of them
}
...
rule clearlog {
   ...
   strings:
      ...
      $s1 = "C:\\Windows\\System32\\cmd.exe /c \"\"" fullword ascii
      $s2 = "logC.dll" fullword ascii
      $s3 = "hhhhh.exe" fullword ascii 
      $s4 = "ttttt.exe" fullword ascii 
      $s5 = "Logger Name:" fullword ascii
      $s6 = "cle.log.1" fullword ascii 
condition:
    any of them
}
...
rule Fireball_gubed {
   ...
   strings:
      $x1 = "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\MRT.exe" fullword ascii 
      $x2 = "tIphlpapi.dll" fullword ascii 
      $x3 = "http://%s/provide?clients=%s&reqs=visit.startload" fullword ascii 
      $x4 = "\\Gubed\\Release\\Gubed.pdb" ascii
      $x5 = "d2hrpnfyb3wv3k.cloudfront.net" fullword ascii 
condition:
    any of them
}
