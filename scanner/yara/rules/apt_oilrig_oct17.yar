...
rule OilRig_Strings_Oct17 {
   ...
   strings:
      $x1 = "%localappdata%\\srvHealth.exe" fullword ascii
      $x2 = "%localappdata%\\srvBS.txt" fullword ascii
...
rule OilRig_ISMAgent_Campaign_Samples1 {
   ...
   strings:
      ...
      $s2 = "C:\\Users\\J-Win-7-32-Vm\\Desktop\\error.jpg" fullword ascii 
      $s3 = "$DATA = [System.Convert]::FromBase64String([IO.File]::ReadAllText('%Base%'));[io.file]::WriteAllBytes(" ascii
      $s4 = " /c echo powershell > " fullword ascii
      $s5 = "\\Libraries\\servicereset.exe" ascii 
      $s6 = "%DestFolder%" fullword ascii
condition:
    any of them
}
...
rule OilRig_ISMAgent_Campaign_Samples2 {
   ...
   strings:
      $x1 = "PolicyConverter.exe" fullword ascii 
      $x2 = "SrvHealth.exe" fullword ascii 
      $x3 = "srvBS.txt" fullword ascii 

      $s1 = "{a3538ba3-5cf7-43f0-bc0e-9b53a98e1643}, PublicKeyToken=3e56350693f7355e" fullword ascii 
      $s2 = "C:\\Windows\\Microsoft.NET\\Framework\\v2.0.50727\\RegAsm.exe" fullword ascii 
condition:
    any of them
}
