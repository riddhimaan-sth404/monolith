...
rule Powerkatz_DLL_Generic {
	...
	strings:
		$s1 = "%3u - Directory '%s' (*.kirbi)" fullword ascii 
		$s2 = "%*s  pPublicKey         : " fullword ascii 
		$s4 = "<3 eo.oe ~ ANSSI E>" fullword ascii 
		$s5 = "\\*.kirbi" ascii 

		$c1 = "kuhl_m_lsadump_getUsersAndSamKey ; kull_m_registry_RegOpenKeyEx SAM Accounts (0x%08x)" fullword ascii 
		$c2 = "kuhl_m_lsadump_getComputerAndSyskey ; kuhl_m_lsadump_getSyskey KO" fullword ascii 
condition:
    any of them
}
