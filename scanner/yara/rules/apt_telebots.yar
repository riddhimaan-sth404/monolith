...
rule TeleBots_KillDisk_1 {
   ...
   strings:
      $s1 = "Plug-And-Play Support Service" fullword ascii 
      $s2 = " /c \"echo Y|" fullword ascii
      $s3 = "-set=06.12.2016#09:30 -est=1410" fullword ascii
      $s4 = "%d.%d.%d#%d:%d" fullword ascii
      $s5 = " /T /C /G " fullword ascii 
      $s6 = "[-] > %ls" fullword ascii 
      $s7 = "[+] > %ls" fullword ascii 
condition:
    any of them
}
...
rule TeleBots_CredRaptor_Password_Stealer {
   ...
   strings:
      ...
      $s6 = "Opera old version credentials" fullword ascii 
condition:
    any of them
}
...
rule TeleBots_Win64_Spy_KeyLogger_G {
   ...
   strings:
      ...
      $s2 = "Install hooks error!" fullword ascii 
      $s4 = "%ls%d.~tmp" fullword ascii 
      $s5 = "[*]Window PID > %d: " fullword ascii 
      $s6 = "Install hooks ok!" fullword ascii 
      $s7 = "[!]Clipboard paste" fullword ascii 
      $s9 = "[*] IMAGE : %ls" fullword ascii 
condition:
    any of them
}
