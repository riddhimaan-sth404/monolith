...
rule Microcin_Sample_3 {
   ...
   strings:
      ...
      $s2 = "test, Version 1.0" fullword ascii 
condition:
    any of them
}
...
rule Microcin_Sample_4 {
   ...
   strings:
      $s1 = "cmd /c dir /a /s \"%s\" > \"%s\"" fullword ascii
      $s2 = "ini.dat" fullword ascii 
      $s3 = "winupdata" fullword ascii 

      $f1 = "%s\\(%08x%08x)%s" fullword ascii 
      $f2 = "%s\\d%08x\\d%08x.db" fullword ascii 
      $f3 = "%s\\u%08x\\u%08x.db" fullword ascii 
      $f4 = "%s\\h%08x\\h%08x.db" fullword ascii 
condition:
    any of them
}
...
rule Microcin_Sample_6 {
   ...
   strings:
      ...
      $s2 = "TEMPDATA" fullword ascii 
      $s3 = "Bruntime error " fullword ascii 
condition:
    any of them
}
