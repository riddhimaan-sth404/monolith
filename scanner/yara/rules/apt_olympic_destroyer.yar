...
rule Destructive_Ransomware_Gen1 {
   ...
   strings:
      $x1 = "/set {default} bootstatuspolicy ignoreallfailures & bcdedit /set {default} recoveryenabled no" fullword ascii 
      $x2 = "delete shadows /all /quiet" fullword ascii 
      $x3 = "delete catalog -quiet" fullword ascii 
condition:
    any of them
}
...
rule OlympicDestroyer_Gen2 {
   ...
   strings:
      $x1 = "cmd.exe /c (ping 0.0.0.0 > nul) && if exist %programdata%\\evtchk.txt" fullword ascii 
      $x2 = "cmd.exe /c (echo strPath = Wscript.ScriptFullName & echo.Set FSO = CreateObject^(\"Scripting.FileSystemObject\"^)" ascii
      $x3 = "del %programdata%\\evtchk.txt" fullword ascii 
      $x4 = "Pyeongchang2018.com\\svc_all_swd_installc" fullword ascii

      $s1 = "<STARTCRED>" fullword ascii 
      $s2 = "SELECT ds_cn FROM ds_computer" fullword ascii 
      $s3 = "\\system32\\notepad.exe" ascii 
