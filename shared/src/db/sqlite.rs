...
impl DatabaseConnection for SqliteConnection {
    async fn execute(&self, sql: &str, params: &[DbParam]) -> Result<u64> {
        ...
        let count = conn
            ...
            .map_err(|e| {
                EdrError::DatabaseError(format!("execute failed: {} - SQL: {}", e, sql))
            })?;
        Ok(count as u64)
    }
    ...
    async fn execute_batch(&self, sql: &str) -> Result<()> {
        ...
        conn.execute_batch(sql).map_err(|e| {
            EdrError::DatabaseError(format!("batch execute failed: {} - SQL: {}", e, sql))
        })?;
        Ok(())
    }
    ...
    async fn query<T: DeserializeOwned + Send>(
        ...
        let mut stmt = conn.prepare(sql).map_err(|e| {
            EdrError::DatabaseError(format!("prepare failed: {} - SQL: {}", e, sql))
        })?;
        ...
        let rows = stmt
            .query_map(
                params_from_iter(rparams.iter().map(|p| p.as_ref())),
                |row| {
        ...
        for row in rows {
            ...
            let entity: T = serde_json::from_value(json_val).map_err(|e| {
                EdrError::DeserializationError(format!("deserialize failed: {}", e))
            })?;
            results.push(entity);
        }
        Ok(results)
...
mod tests {
    ...
    async fn test_sqlite_connect_and_execute() {
        ...
        conn.execute_batch("CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT);")
            .await
            .unwrap();
        conn.execute("INSERT INTO test (name) VALUES (?1)", &["hello".into()])
            .await
            .unwrap();
        let count = conn
            .query_one::<serde_json::Value>("SELECT COUNT(*) as cnt FROM test", &[])
            .await
            .unwrap();
    ...
    async fn test_transaction_rollback() {
        ...
        let conn = db.connect(&config).await.unwrap();
        conn.execute_batch("CREATE TABLE txn_test (id INTEGER PRIMARY KEY, val TEXT);")
            .await
            .unwrap();
        ...
        txn.execute(
            "INSERT INTO txn_test (val) VALUES (?1)",
        ...
        // Verify rollback
        let count = conn
            .query_one::<serde_json::Value>("SELECT COUNT(*) as cnt FROM txn_test", &[])
            .await
            .unwrap();
        let cnt = count
            .unwrap()
