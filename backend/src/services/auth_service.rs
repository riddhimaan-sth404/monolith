...
impl AuthService {
    ...
    pub fn validate_role_transition(&self, current_role: &str, new_role: &str) -> ServiceResult<bool> {
        let hierarchy = ["viewer", "analyst", "administrator"];
        let current_idx = hierarchy.iter().position(|r| *r == current_role).unwrap_or(0);
...
mod tests {
    ...
    fn test_create_auth_context_known_role() {
        ...
        let ctx = service.create_auth_context(
            "user-1".into(),
            "alice".into(),
            "administrator".into(),
        );
    ...
    fn test_validate_role_transition_admin_can_assign_any() {
        let service = AuthService::new();
        assert!(service.validate_role_transition("administrator", "administrator").unwrap());
        assert!(service.validate_role_transition("administrator", "analyst").unwrap());
        assert!(service.validate_role_transition("administrator", "viewer").unwrap());
    }

    #[test]
    fn test_validate_role_transition_analyst_cannot_assign_admin() {
        let service = AuthService::new();
        assert!(!service.validate_role_transition("analyst", "administrator").unwrap());
        assert!(service.validate_role_transition("analyst", "analyst").unwrap());
        assert!(service.validate_role_transition("analyst", "viewer").unwrap());
    }

    #[test]
    fn test_validate_role_transition_viewer_cannot_promote() {
        let service = AuthService::new();
        assert!(!service.validate_role_transition("viewer", "administrator").unwrap());
        assert!(!service.validate_role_transition("viewer", "analyst").unwrap());
        assert!(service.validate_role_transition("viewer", "viewer").unwrap());
    }
}
