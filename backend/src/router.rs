...
pub fn build_router(state: Arc<AppState>) -> Router {
    ...
    let api_router = Router::new()
        ...
        .route("/api/v1/users/mfa/confirm", post(handlers::mfa::confirm_mfa))
        .route("/api/v1/users/mfa/disable", post(handlers::mfa::disable_mfa))
        ...
        .route("/api/v1/endpoints/{id}/isolate", post(handlers::endpoints::isolate))
        .route("/api/v1/endpoints/{id}/release", post(handlers::endpoints::release))
        .route("/api/v1/endpoints/{id}/events", get(handlers::endpoints::events))
        ...
        .route("/api/v1/alerts/memory", get(handlers::alerts::list_memory_alerts))
        .route("/api/v1/alerts/registry-tamper", get(handlers::alerts::list_registry_tamper))
        .route("/api/v1/alerts/{id}", get(handlers::alerts::get))
        .route("/api/v1/alerts/{id}", put(handlers::alerts::update))
        .route("/api/v1/alerts/{id}/suppress", post(handlers::alerts::suppress))
        .route("/api/v1/alerts/{id}/unsuppress", post(handlers::alerts::unsuppress))
        ...
        .route("/api/v1/allowlist/{id}", delete(handlers::allowlist::delete))
        ...
        .route("/api/v1/scans/quarantine", get(handlers::scans::list_quarantine))
        .route("/api/v1/scans/{id}", get(handlers::scans::get))
        .route("/api/v1/scans/{id}", delete(handlers::scans::delete_quarantine))
        .route("/api/v1/scans/{id}/cancel", post(handlers::scans::cancel))
        .route("/api/v1/scans/{id}/restore", post(handlers::scans::restore_quarantine))

        // Reports & Audit Logs
        .route("/api/v1/reports", get(handlers::reports::list))
        .route("/api/v1/reports", post(handlers::reports::generate))
        .route("/api/v1/reports/audit-logs", get(handlers::reports::list_audit_logs))
        .route("/api/v1/reports/{id}", get(handlers::reports::get))
        .route("/api/v1/reports/{id}/download", get(handlers::reports::download))
        ...
        // Settings
        .route("/api/v1/settings/hardware", get(handlers::settings::get_hardware))

        // Scanner reports (internal, from Go scanner)
        .route("/api/v1/scanner/report", post(handlers::scanner::report))

        // Shred
        .route("/api/v1/endpoints/{id}/shred", post(handlers::shred::shred_file))

        // Profiles
        .route("/api/v1/endpoints/{id}/profile", get(handlers::profiles::get_profile))
        .route("/api/v1/endpoints/{id}/profile", put(handlers::profiles::set_profile));

    Router::new()
        .merge(api_router)
        .layer(middleware::from_fn_with_state(state.clone(), auth::auth_middleware))
        .layer(middleware::from_fn_with_state(state.clone(), rate_limit::rate_limit_middleware))
        .layer(middleware::from_fn_with_state(state.clone(), request_id::request_id_middleware))
