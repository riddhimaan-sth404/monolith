...
impl ResponseOrchestrator {
    ...
    async fn terminate_process(&self, action: &ResponseAction) -> Result<ActionExecutionResult> {
        let pid = action.parameters.get("pid")
            .and_then(|v| v.as_u64())
    ...
    async fn restore_quarantine(&self, action: &ResponseAction) -> Result<ActionExecutionResult> {
        let quarantine_id = action.parameters.get("quarantine_id")
            .and_then(|v| v.as_str())
    ...
    async fn trigger_scan(&self, action: &ResponseAction, scan_type: &str) -> Result<ActionExecutionResult> {
        Ok(ActionExecutionResult {
            success: true,
    ...
    async fn update_policy(&self, action: &ResponseAction) -> Result<ActionExecutionResult> {
        let policy_id = action.parameters.get("policy_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| monolith_shared::error::EdrError::ValidationError("policy_id required".into()))?;

        Ok(ActionExecutionResult {
            success: true,
            message: format!("policy update initiated: {} -> {}", action.endpoint_id, policy_id),
