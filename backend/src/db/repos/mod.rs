...
impl Repository<serde_json::Value, String> for UserRepository {
    async fn find_by_id(&self, conn: &dyn DatabaseConnection, id: String) -> Result<Option<serde_json::Value>> {
        conn.query_one_value("SELECT * FROM users WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await
    }

    async fn find_all(&self, conn: &dyn DatabaseConnection) -> Result<Vec<serde_json::Value>> {
        conn.query_value("SELECT * FROM users ORDER BY created_at DESC", &[]).await
    }

    async fn insert(&self, conn: &dyn DatabaseConnection, entity: &serde_json::Value) -> Result<String> {
        let id = entity.get("id").and_then(|v| v.as_str()).unwrap_or_default().to_string();
        conn.execute(
    ...
    async fn update(&self, conn: &dyn DatabaseConnection, entity: &serde_json::Value) -> Result<bool> {
        let id = entity.get("id").and_then(|v| v.as_str()).unwrap_or_default().to_string();
    ...
    async fn delete(&self, conn: &dyn DatabaseConnection, id: String) -> Result<bool> {
        let affected = conn.execute("DELETE FROM users WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await?;
        Ok(affected > 0)
    }

    async fn count(&self, conn: &dyn DatabaseConnection) -> Result<u64> {
        let result = conn.query_one_value("SELECT COUNT(*) as cnt FROM users", &[]).await?;
        Ok(result.and_then(|r| r.get("cnt").and_then(|v| v.as_i64()).map(|i| i as u64)).unwrap_or(0))
    }

    async fn exists(&self, conn: &dyn DatabaseConnection, id: String) -> Result<bool> {
        let result = conn.query_one_value("SELECT 1 as exists_flag FROM users WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await?;
        Ok(result.is_some())
    }
}
...
impl Repository<serde_json::Value, String> for EndpointRepository {
    async fn find_by_id(&self, conn: &dyn DatabaseConnection, id: String) -> Result<Option<serde_json::Value>> {
        conn.query_one_value("SELECT * FROM endpoints WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await
    }

    async fn find_all(&self, conn: &dyn DatabaseConnection) -> Result<Vec<serde_json::Value>> {
        conn.query_value("SELECT * FROM endpoints ORDER BY last_seen DESC", &[]).await
    }
    ...
    async fn delete(&self, conn: &dyn DatabaseConnection, id: String) -> Result<bool> {
        let affected = conn.execute("DELETE FROM endpoints WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await?;
        Ok(affected > 0)
    }

    async fn count(&self, conn: &dyn DatabaseConnection) -> Result<u64> {
        let result = conn.query_one_value("SELECT COUNT(*) as cnt FROM endpoints", &[]).await?;
        Ok(result.and_then(|r| r.get("cnt").and_then(|v| v.as_i64()).map(|i| i as u64)).unwrap_or(0))
    }

    async fn exists(&self, conn: &dyn DatabaseConnection, id: String) -> Result<bool> {
        let result = conn.query_one_value("SELECT 1 as exists_flag FROM endpoints WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await?;
        Ok(result.is_some())
    }
}
...
impl Repository<serde_json::Value, String> for AlertRepository {
    async fn find_by_id(&self, conn: &dyn DatabaseConnection, id: String) -> Result<Option<serde_json::Value>> {
        conn.query_one_value("SELECT * FROM alerts WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await
    }

    async fn find_all(&self, conn: &dyn DatabaseConnection) -> Result<Vec<serde_json::Value>> {
        conn.query_value("SELECT * FROM alerts ORDER BY created_at DESC", &[]).await
    }
    ...
    async fn update(&self, conn: &dyn DatabaseConnection, entity: &serde_json::Value) -> Result<bool> {
        ...
        let affected = conn.execute(
            ...
            &[
                monolith_shared::db::DbParam::Text(entity.get("status").and_then(|v| v.as_str()).unwrap_or("new").to_string()),
                monolith_shared::db::DbParam::Text(id),
            ],
        ).await?;
        Ok(affected > 0)
    ...
    async fn delete(&self, conn: &dyn DatabaseConnection, id: String) -> Result<bool> {
        let affected = conn.execute("DELETE FROM alerts WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await?;
        Ok(affected > 0)
    }

    async fn count(&self, conn: &dyn DatabaseConnection) -> Result<u64> {
        let result = conn.query_one_value("SELECT COUNT(*) as cnt FROM alerts", &[]).await?;
        Ok(result.and_then(|r| r.get("cnt").and_then(|v| v.as_i64()).map(|i| i as u64)).unwrap_or(0))
    }

    async fn exists(&self, conn: &dyn DatabaseConnection, id: String) -> Result<bool> {
        let result = conn.query_one_value("SELECT 1 as exists_flag FROM alerts WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await?;
        Ok(result.is_some())
    }
}
...
impl Repository<serde_json::Value, String> for AllowlistRepository {
    async fn find_by_id(&self, conn: &dyn DatabaseConnection, id: String) -> Result<Option<serde_json::Value>> {
        conn.query_one_value("SELECT * FROM allowlist WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await
    }

    async fn find_all(&self, conn: &dyn DatabaseConnection) -> Result<Vec<serde_json::Value>> {
        conn.query_value("SELECT * FROM allowlist ORDER BY created_at DESC", &[]).await
    }
    ...
    async fn insert(&self, conn: &dyn DatabaseConnection, entity: &serde_json::Value) -> Result<String> {
        ...
        conn.execute(
            ...
            &[
                monolith_shared::db::DbParam::Text(id.clone()),
                monolith_shared::db::DbParam::Text(entity.get("rule_type").and_then(|v| v.as_str()).unwrap_or("").to_string()),
                monolith_shared::db::DbParam::Text(entity.get("value").and_then(|v| v.as_str()).unwrap_or("").to_string()),
                monolith_shared::db::DbParam::Text(entity.get("description").and_then(|v| v.as_str()).unwrap_or("").to_string()),
            ],
        ).await?;
        Ok(id)
    ...
    async fn delete(&self, conn: &dyn DatabaseConnection, id: String) -> Result<bool> {
        let affected = conn.execute("DELETE FROM allowlist WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await?;
        Ok(affected > 0)
    }

    async fn count(&self, conn: &dyn DatabaseConnection) -> Result<u64> {
        let result = conn.query_one_value("SELECT COUNT(*) as cnt FROM allowlist", &[]).await?;
        Ok(result.and_then(|r| r.get("cnt").and_then(|v| v.as_i64()).map(|i| i as u64)).unwrap_or(0))
    }

    async fn exists(&self, conn: &dyn DatabaseConnection, id: String) -> Result<bool> {
        let result = conn.query_one_value("SELECT 1 as exists_flag FROM allowlist WHERE id = ?1", &[monolith_shared::db::DbParam::Text(id)]).await?;
        Ok(result.is_some())
    }
}
