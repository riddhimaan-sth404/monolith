...
pub async fn summary(
    ...
    let active_iocs = state
        .db
        .query_raw("SELECT COUNT(*) as cnt FROM iocs", &[])
        .await
        .map_err(|e| {
            (
                axum::http::StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let ioc_count = active_iocs
        .first()
        .and_then(|r| r.get(0))
        .and_then(|v| v.as_i64())
        .unwrap_or(0);

    let events_today = state
        .db
        .query_raw(
    ...
    let (agent_running, hostname) = endpoints
        .first()
        .map(|r| {
    ...
    let active_scans_count = active_scans
        .first()
        .and_then(|r| r.get(0))
        .and_then(|v| v.as_i64())
        .unwrap_or(0);
    ...
    let last_completed = last_done
        ...
        .map(|r| {
            ...
            let threats: Vec<Value> = all_results
                .iter()
                .filter(|r| {
                    r.get("verdict")
                        .and_then(|v| v.as_str())
            ...
            let (heuristic_avg, ember_avg, fusion_avg) = if n > 0.0 {
                let h: f64 = all_results
                    .iter()
                    .filter_map(|r| r.get("heuristic_score").and_then(|v| v.as_f64()))
                    .sum();
                let e: f64 = all_results
                    .iter()
                    .filter_map(|r| r.get("ember_score").and_then(|v| v.as_f64()))
                    .sum();
                let f: f64 = all_results
                    .iter()
                    .filter_map(|r| r.get("fusion_score").and_then(|v| v.as_f64()))
                    .sum();
                (h / n, e / n, f / n)
            } else {
                (0.0, 0.0, 0.0)
    ...
    let top_alerts_list: Vec<Value> = top_alerts
        .iter()
        .map(|r| {
            json!({
...
mod tests {
    ...
    fn parses_sqlite_style_scan_timestamps() {
        assert_eq!(
            compute_duration_secs("2026-07-12 03:05:26", "2026-07-12 03:05:50"),
            24
        );
    }
    ...
    fn parses_rfc3339_scan_timestamps() {
        assert_eq!(
            compute_duration_secs("2026-07-12T03:05:26Z", "2026-07-12T03:05:50Z"),
            24
        );
    }
}
