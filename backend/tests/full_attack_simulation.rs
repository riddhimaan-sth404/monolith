...
async fn setup_db() -> Box<dyn DatabaseConnection> {
    ...
    let conn = db
        .connect(&monolith_shared::config::DatabaseConfig {
...
async fn seed_iocs(db: &dyn DatabaseConnection, engine: &DetectionEngine) {
    ...
    let rows = db
        .query_value("SELECT ioc_type, value FROM iocs", &[])
        .await
        .unwrap();
    let ioc_values: Vec<Value> = rows
        .into_iter()
        .map(|r| {
            json!({
...
fn build_alert_info(result: &DetectionResult, endpoint_id: &str) -> response_rules::AlertInfo {
    ...
    let sources: Vec<response_rules::DetectionSource> = result
        .tags
        .iter()
    ...
    ) -> Vec<monolith_backend::engine::detection::DetectionResult>,
{
    ...
    assert!(
        !results.is_empty(),
        "{}: expected detection, got none",
        phase_name
    );
    eprintln!(
        "  [DETECTED] {} â€” rule={} severity={} score={}",
    ...
    assert!(
        results[0].rule_id.starts_with(expected_rule_prefix),
...
async fn test_phase_brute_force() {
    ...
    run_phase(
        &*db,
        "brute_force",
        "correlation_brute_force",
        |engine, _db| {
            // Need >=5 user_logon events before detection triggers
            for _ in 0..6 {
                let event = make_event(
                    "user_logon",
                    json!({"username": "admin", "status": "failed"}),
                );
                let results = engine.evaluate_event(&event);
                if !results.is_empty() {
                    return results;
...
async fn test_phase_persistence_task() {
    ...
    run_phase(
        ...
        "correlation_persistence",
        |engine, _| {
            engine.evaluate_event(&make_event(
                "scheduled_task",
                json!({
...
async fn test_phase_credential_dumping_procdump() {
    ...
    run_phase(
        &*db,
        "credential_procdump",
        "correlation_credential_dumping",
        |engine, _| {
            engine.evaluate_event(&make_event(
                "process_create",
                json!({
...
async fn test_phase_registry_hive_copy() {
    ...
    run_phase(
        &*db,
        "registry_hive_copy",
        "correlation_registry_hive_copy",
        |engine, _| {
            engine.evaluate_event(&make_event(
                "file_create",
                json!({
...
async fn test_phase_discovery_systeminfo() {
    ...
    run_phase(
        &*db,
        "system_discovery",
        "correlation_discovery_sequence",
        |engine, _| {
            engine.evaluate_event(&make_event(
                "process_create",
                json!({
...
async fn test_phase_exfiltration() {
    ...
    for _ in 0..3 {
        ...
        if !result.is_empty() {
            ...
            eprintln!(
                "  [DETECTED] exfiltration â€” rule={} score={}",
                result[0].rule_id, result[0].score
            );
            assert!(
                result[0]
                    .rule_id
                    .starts_with("correlation_data_exfiltration"),
                "Expected exfiltration rule"
            );
...
async fn test_phase_indicator_removal_wevtutil() {
    ...
    run_phase(
        &*db,
        "indicator_removal_wevtutil",
        "correlation_indicator_removal",
        |engine, _| {
            engine.evaluate_event(&make_event(
                "process_create",
                json!({
...
async fn test_phase_remote_access_teamviewer() {
    ...
    run_phase(
        &*db,
        "remote_access_teamviewer",
        "correlation_remote_access_",
        |engine, _| {
            engine.evaluate_event(&make_event(
                "process_create",
                json!({
...
async fn test_phase_ioc_sha256() {
    ...
    let results = engine.evaluate_event(&make_event(
        "file_create",
        json!({
    ...
    assert!(
        results[0].rule_id.starts_with("ioc_match"),
        "Expected IOC rule"
    );
    eprintln!(
        "  [DETECTED] ioc_sha256 â€” rule={} score={}",
        results[0].rule_id, results[0].score
    );
    ...
    for a in &actions {
        eprintln!(
            "    [AUTO-RESPONSE] {} â†’ {:?} (KillAndQuarantine)",
            a.rule_id, a.action
        );
    }
...
async fn test_phase_ioc_domain() {
    ...
    eprintln!(
        "  [DETECTED] ioc_domain â€” rule={} score={}",
        results[0].rule_id, results[0].score
    );
...
async fn test_phase_network_scanning() {
    ...
    assert!(
        results[0]
            .rule_id
            .starts_with("correlation_network_scanning"),
        "Expected scanning rule"
    );
    eprintln!(
        "  [DETECTED] network_scanning â€” rule={} score={}",
        results[0].rule_id, results[0].score
    );
...
async fn test_phase_suspicious_powershell() {
    ...
    run_phase(
        ...
        "correlation_powershell_suspicious",
        |engine, _| {
            engine.evaluate_event(&make_event(
                "process_create",
                json!({
...
async fn test_phase_response_rules_all_actions() {
    ...
    let mut bf = None;
    for _ in 0..10 {
        let r = engine.evaluate_event(&make_event(
            "user_logon",
            json!({"username":"admin","status":"failed"}),
        ));
        if !r.is_empty() {
    ...
    if let Some(r) = bf {
        check_result(r, "brute_force");
    }

    // Credential dumping
    let engine = DetectionEngine::new();
    seed_iocs(&*db, &engine).await;
    if let Some(r) = e_dump(&engine).into_iter().last() {
        check_result(r, "cred_dump");
    }

    // LOLBin
    let engine = DetectionEngine::new();
    seed_iocs(&*db, &engine).await;
    if let Some(r) = e_lolbin(&engine).into_iter().last() {
        check_result(r, "lolbin");
    }

    // IOC
    let engine = DetectionEngine::new();
    seed_iocs(&*db, &engine).await;
    if let Some(r) = e_ioc(&engine).into_iter().last() {
        check_result(r, "ioc");
    }
    ...
    let engine = DetectionEngine::new();
    seed_iocs(&*db, &engine).await;
    if let Some(r) = e_masq(&engine).into_iter().last() {
        check_result(r, "masq");
    }
    ...
    let engine = DetectionEngine::new();
    seed_iocs(&*db, &engine).await;
    if let Some(r) = e_indicator_removal(&engine).into_iter().last() {
        check_result(r, "indicator_removal");
    }

    assert!(
        total_actions >= 4,
        "Expected >=4 response actions, got {}",
        total_actions
    );
    eprintln!(
        "  [OK] Phase 14: Response Rules â€” {} actions triggered",
        total_actions
    );
}
